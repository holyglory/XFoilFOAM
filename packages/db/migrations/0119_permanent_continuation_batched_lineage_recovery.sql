-- Migration 0118 correctly refused a source when its result and attempt
-- engine identifiers disagreed.  Some older case-parallel PRECALC batches
-- intentionally retained the parent engine id on result_attempts while the
-- result row retained the per-AoA child engine id.  This correction accepts
-- only that explicit batch representation; it never relaxes the ordinary
-- one-to-one source rule and it changes no result or attempt evidence.
WITH eligible AS (
  SELECT DISTINCT
    obligation."id" AS obligation_id,
    terminal_job."id" AS terminal_job_id
  ...
  WHERE obligation."state" = 'blocked'
    ...
    AND terminal_job."request_payload" ->> 'uransFidelity' = 'precalc'
    AND CASE
    ...
    AND continuation_source_job."status" IN ('done', 'failed', 'cancelled')
    ...
    AND continuation_source_job."bc_ids" @>
      jsonb_build_array(continuation_source_attempt."bc_id")
    -- The parent request is immutable evidence that this exact result attempt
    -- is an AoA child of this exact obligation, not merely a same-cell row.
    AND continuation_source_job."request_payload" ->> 'uransFidelity' = 'precalc'
    AND CASE
      WHEN jsonb_typeof(
        continuation_source_job."request_payload" -> 'precalcObligationIds'
      ) = 'array'
      THEN continuation_source_job."request_payload" -> 'precalcObligationIds' @>
        jsonb_build_array(obligation."id"::text)
      ELSE false
    END
    AND CASE
      WHEN jsonb_typeof(continuation_source_job."request_payload" -> 'aoas') = 'array'
      THEN continuation_source_job."request_payload" -> 'aoas' @>
    ...
    END
    AND CASE
      WHEN continuation_source_result."solver_implementation_id" =
        '2f8bc764-09ae-4ff3-8fd2-000000000000'::uuid
      THEN '2f8bc764-09ae-4ff3-8fd2-240600000001'::uuid
      ELSE continuation_source_result."solver_implementation_id"
    END = CASE
      WHEN target_revision."solver_implementation_id" =
        '2f8bc764-09ae-4ff3-8fd2-000000000000'::uuid
      THEN '2f8bc764-09ae-4ff3-8fd2-240600000001'::uuid
    ...
    AND NULLIF(btrim(continuation_source_result."engine_job_id"), '') IS NOT NULL
  ...
  SET
    ...
    "updatedAt" = now()
  FROM eligible
  WHERE obligation."id" = eligible.obligation_id
  RETURNING obligation."id", eligible.terminal_job_id
)
...
JOIN "sim_precalc_obligation_requests" coverage
  ON coverage."obligation_id" = reopened."id"
JOIN "sim_precalc_obligation_attempts" terminal_submission
  ON terminal_submission."obligation_id" = reopened."id"
JOIN "sim_jobs" terminal_job
  ON terminal_job."id" = terminal_submission."sim_job_id"
  AND terminal_job."id" = reopened.terminal_job_id
JOIN "sim_precalc_obligations" obligation
  ON obligation."id" = reopened."id"
