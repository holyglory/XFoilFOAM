...
beforeAll(async () => {
  ...
  await client.unsafe(`
    ...
    INSERT INTO result_interpretations
      (id, result_id, result_attempt_id, reducer_version_id, source,
       input_evidence_signature, state, regime, selected_window,
       terminal_reason, statistics, diagnostics, cl, cd, cm, cl_cd, cl_cd_interval_state,
       uncertainty_basis, effective_blocks)
    VALUES
      ('${ID.interpretation}', '${ID.result}', '${ID.attempt}', '${ID.reducer}',
      ...
      ('${ID.terminalInterpretation}', '${ID.result}', '${ID.attempt}', '${ID.reducer}',
      ...
      ('${ID.interpretationCascade}', '${ID.resultCascade}', '${ID.attemptCascade}', '${ID.reducer}',
...
describe("0096 result interpretation ledger reconciliation", () => {
  it("is idempotent when the exact 0096 reconciliation SQL is run twice against a <=0096 database", async () => {
    ...
    // This is deliberately not a second Drizzle invocation: the regression
    // targets the DDL's own IF NOT EXISTS/DO reconciliation contract after a
    // crash/retry, while the journal still records <=0096.
    await client.unsafe(reconciliationSql);
    await client.unsafe(reconciliationSql);

    const [shape] = await client<
      Array<{ tables: number; legacyIndex: boolean; currentPointers: number }>
    >`
      SELECT
        (
          SELECT count(*)::int
        ...
        to_regclass('public.result_interpretations_attempt_reducer_evidence_uq')
          IS NOT NULL AS "legacyIndex",
        (
          SELECT count(*)::int
    ...
    `;
    expect(shape).toEqual({ tables: 7, legacyIndex: true, currentPointers: 2 });
  });
