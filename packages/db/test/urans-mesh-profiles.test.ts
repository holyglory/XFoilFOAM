...
const validPlanInput = (
  numerics: Partial<CampaignPlanInput["numerics"]> = {},
): CampaignPlanInput => ({
...
describe("per-tier URANS mesh physics hashing", () => {
  it("keeps null/absent URANS mesh pins byte-identical and changes when a tier mesh is pinned", () => {
    const {
      uransMesh: _uransMesh,
      uransPrecalcMesh: _uransPrecalcMesh,
      ...legacySnapshot
    } = baseSnapshot;
    const legacyHash = physicsHashForSnapshot(
      legacySnapshot as unknown as SimulationSetupSnapshot,
    );
...
describe("campaign plan numerics normalization", () => {
  ...
  it("rejects non-string optional URANS mesh profile IDs", () => {
    const input = validPlanInput({
      uransMeshProfileId: 42 as unknown as string,
    });
...
describe("loadNumericsRows optional URANS mesh lookups", () => {
  it("errors clearly when an optional URANS mesh profile id is missing", async () => {
    ...
    const fakeDb = {
      select: () => ({
        from: (table: unknown) => ({
          where: () => ({
            limit: async () => {
              if (table === boundaryProfiles)
                return [{ id: "boundary-profile" }];
              if (table === solverProfiles)
                return [
                  {
                    id: "solver-profile",
                    solverImplementationId: "solver-implementation",
                  },
                ];
              if (table === solverImplementations)
                return [{ id: "solver-implementation" }];
    ...
    await expect(
      ...
      code: "not_found",
      message: "URANS mesh profile not found",
    });
  });
});
