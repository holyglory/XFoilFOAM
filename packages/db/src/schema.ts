...
/**
 * Actual resolved mesh identity, distinct from requested mesh profile.  A
 * row exists only when exact mesh bytes were reported by the engine or
 * reconstructed from authenticated archived evidence; unknown meshes remain
 * honestly absent rather than sharing a synthetic identity.
 */
export const resultAttemptMeshIdentities = pgTable(
  "result_attempt_mesh_identities",
  {
    ...
    sourceEvidenceSignature: text("source_evidence_signature").notNull(),
  ...
  (t) => ({
    attemptOwnerFk: foreignKey({
      columns: [t.resultAttemptId, t.resultId],
      foreignColumns: [resultAttempts.id, resultAttempts.resultId],
      name: "result_attempt_mesh_identities_attempt_owner_fk",
    }).onDelete("cascade"),
    archiveOwnerFk: foreignKey({
      ...
      name: "result_attempt_mesh_identities_archive_owner_fk",
    }).onDelete("restrict"),
    attemptFingerprintUq: uniqueIndex(
      "result_attempt_mesh_identities_attempt_fingerprint_uq",
    ).on(t.resultAttemptId, t.fingerprintVersion),
    idAttemptUq: unique("result_attempt_mesh_identities_id_attempt_uq").on(
      t.id,
      t.resultAttemptId,
    ),
    contentIdx: index("result_attempt_mesh_identities_content_idx").on(
      t.fingerprintVersion,
      t.contentSha256,
    ),
    resultIdx: index("result_attempt_mesh_identities_result_idx").on(
      t.resultId,
      t.createdAt,
    ),
    fingerprintVersionCheck: check(
      "result_attempt_mesh_identities_fingerprint_version_check",
      sql`${t.fingerprintVersion} > 0`,
    ),
    shaCheck: check(
      "result_attempt_mesh_identities_sha_check",
      sql`${t.contentSha256} ~ '^[0-9a-f]{64}$' AND ${t.recipeSha256} ~ '^[0-9a-f]{64}$'`,
    ),
    resolvedParamsCheck: check(
      "result_attempt_mesh_identities_resolved_params_check",
      sql`jsonb_typeof(${t.resolvedParams}) = 'object'`,
    ),
    sourceCheck: check(
      "result_attempt_mesh_identities_source_check",
      sql`${t.source} IN ('engine_reported', 'archive_backfill') AND btrim(${t.sourceEvidenceSignature}) <> '' AND ((${t.source} = 'archive_backfill' AND ${t.sourceArchiveId} IS NOT NULL) OR (${t.source} = 'engine_reported' AND ${t.sourceArchiveId} IS NULL))`,
    ),
  }),
);
...
export const resultInterpretations = pgTable(
  "result_interpretations",
  {
    ...
    meshIdentityId: uuid("mesh_identity_id"),
    sourceArchiveId: uuid("source_archive_id"),
    source: text("source").notNull(),
  ...
  (t) => ({
    ...
    meshOwnerFk: foreignKey({
      columns: [t.meshIdentityId, t.resultAttemptId],
      foreignColumns: [
        resultAttemptMeshIdentities.id,
        resultAttemptMeshIdentities.resultAttemptId,
      ],
      name: "result_interpretations_mesh_owner_fk",
    }).onDelete("restrict"),
    archiveOwnerFk: foreignKey({
      columns: [t.sourceArchiveId, t.resultAttemptId],
      foreignColumns: [
    ...
    attemptReducerEvidenceUq: uniqueIndex(
      "result_interpretations_attempt_reducer_evidence_uq",
...
export const simPrecalcObligations = pgTable(
  "sim_precalc_obligations",
  {
    ...
    remediationAttemptsGranted: integer("remediation_attempts_granted")
      .notNull()
      .default(0),
    remediationReason: text("remediation_reason"),
    remediationSourceRevision: text("remediation_source_revision"),
    remediationGrantedAt: timestamp("remediation_granted_at", {
      withTimezone: true,
    }),
  ...
  (t) => ({
    ...
    remediationShapeCheck: check(
      ...
      sql`(
        ...
        AND ${t.remediationReason} IS NULL
...
export type ResultAttemptMeshIdentity =
  typeof resultAttemptMeshIdentities.$inferSelect;
