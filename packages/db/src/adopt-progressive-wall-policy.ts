...
import { adoptProgressiveWallPolicy } from "./progressive-recipe-adoption";
...
if (
  ...
  throw new Error(
    "Usage: adopt-progressive-wall-policy CAMPAIGN_UUID [--dry-run|--apply]",
  );
const { db, sql } = createClient({ max: 1 });
const rollback = new Error("reviewed dry-run rollback");
let result: Awaited<ReturnType<typeof adoptProgressiveWallPolicy>> | undefined;
try {
  await db
    .transaction(async (transaction) => {
      result = await adoptProgressiveWallPolicy(
