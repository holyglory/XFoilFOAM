// Fidelity-ladder classifier gates (POLAR_CLASSIFIER_VERSION fidelity-ladder-v4):
...
describe("fidelity-aware frame-track period bar (v4)", () => {
  it("pins the version stamp and the per-tier bars", () => {
    expect(POLAR_CLASSIFIER_VERSION).toBe("fidelity-ladder-v4");
  ...
  it("MUST-CATCH: a precalc row with 3 retained periods ACCEPTS; the same track at full/legacy fidelity REJECTS", () => {
    ...
    const precalc = classifyPolarEvidence([{ ...uransRow, fidelity: "urans_precalc", frameTrack: threePeriods }]);
    ...
    for (const fidelity of ["urans_full", null, undefined] as const) {
      const full = classifyPolarEvidence([{ ...uransRow, fidelity, frameTrack: threePeriods }]);
  ...
  it("a precalc row below 3 periods still rejects (the precalc bar is 3, not 0)", () => {
    const classified = classifyPolarEvidence([
      { ...uransRow, fidelity: "urans_precalc", frameTrack: { ...contractFrameTrack, periods_retained: 2 } },
    ]);
    expect(classified.classifications[0].state).toBe("rejected");
    expect(classified.classifications[0].reasons).toContain("insufficient-periods");
  });

  it("the stationarity gate itself is unchanged: a non-stationary precalc track rejects", () => {
    const classified = classifyPolarEvidence([
      { ...uransRow, fidelity: "urans_precalc", frameTrack: { ...contractFrameTrack, periods_retained: 3, stationary: false } },
    ]);
    expect(classified.classifications[0].state).toBe("rejected");
  ...
  it("full-fidelity rows at >= 5 periods keep accepting (no regression)", () => {
    const classified = classifyPolarEvidence([
      { ...uransRow, fidelity: "urans_full", frameTrack: { ...contractFrameTrack, periods_retained: 5 } },
    ]);
...
describe("oscillating-steady acceptance (steady_history.mean_stable, v4)", () => {
  ...
  it("fail closed: mean_stable false / drifted string / absent history all still reject not-converged", () => {
    for (const history of [
      ...
      const classified = classifyPolarEvidence([{ ...oscillatingSteadyRow, steadyHistory: history as never }]);
      expect(classified.classifications[0].state).toBe("rejected");
  ...
  it("the waiver is narrow: other gates still reject an oscillating-steady row (error, missing coefficients, non-physical)", () => {
    const withError = classifyPolarEvidence([{ ...oscillatingSteadyRow, error: "boom" }]);
    expect(withError.classifications[0].state).toBe("rejected");
    expect(withError.classifications[0].reasons).toContain("solver-error");

    const missing = classifyPolarEvidence([{ ...oscillatingSteadyRow, cm: null }]);
    expect(missing.classifications[0].state).toBe("rejected");
    expect(missing.classifications[0].reasons).toContain("missing-coefficients");

    const blown = classifyPolarEvidence([{ ...oscillatingSteadyRow, cl: 79.8 }]);
    expect(blown.classifications[0].state).toBe("rejected");
    expect(blown.classifications[0].reasons).toContain("non-physical-coefficients");
  });
  ...
  it("an UNSTEADY row never uses the steady waiver (isOscillatingSteadyStable is steady-only)", () => {
    ...
    expect(isOscillatingSteadyStable({ ...oscillatingSteadyRow, unsteady: true })).toBe(false);
    const classified = classifyPolarEvidence([
      { ...uransRow, converged: false, steadyHistory, frameTrack: contractFrameTrack },
    ]);
    expect(classified.classifications[0].state).toBe("rejected");
    expect(classified.classifications[0].reasons).toContain("not-converged");
  });
