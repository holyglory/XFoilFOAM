/** Strict, dependency-free proof contract for a physically non-shedding
 * URANS observation.  This lives in core so ingestion and persisted evidence
 * classification use the same fail-closed parser. */
...
/** Callers separately distinguish legacy `undefined` and current `null`. */
export function parseNoSheddingCertificate(
  ...
  for (const key of [
    ...
    if (!positiveFinite(value[key])) {
      errors.push(
        `no_shedding_certificate.${key}: expected positive finite number`,
      );
    }
  }
  for (const key of [
    ...
    if (!finite(value[key]) || value[key] < 0) {
      errors.push(
        `no_shedding_certificate.${key}: expected non-negative finite number`,
      );
    }
  }
  for (const key of [
  ...
  for (const key of [
    ...
    if (finite(value[key]) && value[key] < 0) {
      errors.push(
        `no_shedding_certificate.${key}: expected non-negative number`,
      );
    }
  }
  if (
