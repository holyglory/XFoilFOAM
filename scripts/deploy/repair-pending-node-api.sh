...
OPENCFD_2406_IMPLEMENTATION_ID="2f8bc764-09ae-4ff3-8fd2-240600000001"
...
read_env_var() {
  ...
  ' "$ENV_FILE")"; then
    fail "Deployment environment must contain exactly one $key value." 14
    return $?
  fi
  printf '%s\n' "$value"
}
...
if [[ "$staging_real/" == "$app_real/"* || "$app_real/" == "$staging_real/"* ]]; then
  fail "Pending-cutover repair requires sibling staging and bound release trees." 2
  exit $?
fi
...
if [[ "$(stat -c '%a' "$AIRFOILS_PRO_STATE_DIR")" != "700" || \
      "$(stat -c '%u' "$AIRFOILS_PRO_STATE_DIR")" != "$(id -u)" ]]; then
  fail "Pending-cutover repair requires an owner-only mode-0700 state directory." 2
  exit $?
fi
if [[ "$REPAIR_RECEIPT_FILE" != "$AIRFOILS_PRO_STATE_DIR/pending-cutover-node-api-repair.json" || \
      "$OPENCFD2606_CANARY_RECEIPT_FILE" != "$AIRFOILS_PRO_STATE_DIR/openfoam-2606-canary-receipt.pending.json" ]]; then
  fail "Pending-cutover repair state files must use their canonical protected paths." 2
  exit $?
fi
if [[ -L "$REPAIR_RECEIPT_FILE" || -L "$OPENCFD2606_CANARY_RECEIPT_FILE" ]]; then
  fail "Pending-cutover repair state files must never be symbolic links." 2
  exit $?
fi
...
WITH target_cutovers AS (
  SELECT COALESCE(json_agg(json_build_object(
    'id', c.id,
    'campaignId', c.campaign_id,
    'campaignStatus', campaign.status,
    'campaignCurrentGeneration', campaign.current_condition_generation,
    'campaignCurrentPlanRevisionId', campaign.current_plan_revision_id,
    'fromSolverImplementationId', c.from_solver_implementation_id,
    'toSolverImplementationId', c.to_solver_implementation_id,
    'status', c.status,
    'canaryAttestationId', c.canary_attestation_id,
    'sourcePlanRevisionId', c.source_plan_revision_id,
    'targetPlanRevisionId', c.target_plan_revision_id,
    'sourceGeneration', c.source_generation,
    'targetGeneration', c.target_generation,
    'priorCampaignStatus', c.prior_campaign_status,
    'sourceConditionCount', c.source_condition_count,
    'targetConditionCount', c.target_condition_count,
    'targetPointCount', c.target_point_count,
    'finalizedAt', c.finalized_at,
    'completedAt', c.completed_at
  ) ORDER BY c.id), '[]'::json) AS rows
  FROM sim_campaign_solver_cutovers c
  JOIN sim_campaigns campaign ON campaign.id = c.campaign_id
  WHERE c.to_solver_implementation_id = '$OPENCFD_2606_IMPLEMENTATION_ID'
)
SELECT json_build_object(
  'sourcePoolRows', (SELECT count(*) FROM solver_execution_pools WHERE id = '$OPENCFD_2406_POOL_ID'),
  'sourcePoolEnabled', (SELECT enabled FROM solver_execution_pools WHERE id = '$OPENCFD_2406_POOL_ID'),
  'sourcePoolImplementationId', (SELECT solver_implementation_id FROM solver_execution_pools WHERE id = '$OPENCFD_2406_POOL_ID'),
  'sourceImplementationRows', (SELECT count(*) FROM solver_implementations WHERE id = '$OPENCFD_2406_IMPLEMENTATION_ID'),
  'sourceImplementationRetired', (SELECT retired_at IS NOT NULL FROM solver_implementations WHERE id = '$OPENCFD_2406_IMPLEMENTATION_ID'),
  'targetPoolRows', (SELECT count(*) FROM solver_execution_pools WHERE id = '$OPENCFD_2606_POOL_ID'),
  'targetPoolEnabled', (SELECT enabled FROM solver_execution_pools WHERE id = '$OPENCFD_2606_POOL_ID'),
  'targetPoolImplementationId', (SELECT solver_implementation_id FROM solver_execution_pools WHERE id = '$OPENCFD_2606_POOL_ID'),
  'cutovers', (SELECT rows FROM target_cutovers),
  'targetAttestationCount', (
    SELECT count(*) FROM solver_engine_canary_attestations
...
p = json.load(sys.stdin)
rows = p.get("cutovers")
source_impl, target_impl = sys.argv[1:]
if (
    p.get("sourcePoolRows") != 1
    or p.get("sourcePoolEnabled") is not False
    or p.get("sourcePoolImplementationId") != source_impl
    or p.get("sourceImplementationRows") != 1
    or p.get("sourceImplementationRetired") is not True
):
    raise SystemExit("OpenCFD v2406 source execution remains available or has wrong ownership")
if (
    p.get("targetPoolRows") != 1
    or p.get("targetPoolEnabled") is not False
    or p.get("targetPoolImplementationId") != target_impl
):
    raise SystemExit("OpenCFD v2606 execution pool is not uniquely disabled")
if p.get("targetAttestationCount") != 0:
    raise SystemExit("an OpenCFD v2606 canary attestation already exists")
if not isinstance(rows, list) or len(rows) != 1:
    raise SystemExit("expected exactly one prepared target cutover")
row = rows[0]
if (
    row.get("status") != "prepared"
    or row.get("campaignStatus") != "paused"
    or row.get("fromSolverImplementationId") != source_impl
    or row.get("toSolverImplementationId") != target_impl
    or row.get("campaignCurrentGeneration") != row.get("sourceGeneration")
    or row.get("campaignCurrentPlanRevisionId") != row.get("sourcePlanRevisionId")
    or row.get("targetGeneration") != row.get("sourceGeneration") + 1
    or row.get("priorCampaignStatus") not in {"active", "paused", "attention"}
    or not isinstance(row.get("sourceConditionCount"), int)
    or row.get("sourceConditionCount") <= 0
    or row.get("targetConditionCount") != 0
    or row.get("targetPointCount") != 0
    or any(
...
' "$OPENCFD_2406_IMPLEMENTATION_ID" "$OPENCFD_2606_IMPLEMENTATION_ID"
}
...
if ! curl -fsS --max-time 5 http://127.0.0.1:4000/health >/dev/null; then
  fail "Pending-cutover repair requires a healthy bound Node API before replacement." 12
  exit $?
fi
node_before_api_sha="$(docker exec "$node_before" sha256sum /app/apps/api/src/admin-routes.ts | awk '{print $1}')"
if [[ -e "$REPAIR_RECEIPT_FILE" || -L "$REPAIR_RECEIPT_FILE" ]]; then
  if [[ "$node_before_api_sha" != "$EXPECTED_BOUND_API_SHA256" && \
        "$node_before_api_sha" != "$EXPECTED_REPAIR_API_SHA256" ]]; then
    fail "Journaled Node API bytes match neither the bound nor reviewed repair implementation." 14
    exit $?
  fi
elif [[ "$node_before_api_sha" != "$EXPECTED_BOUND_API_SHA256" ]]; then
  fail "Running Node API bytes do not match the source-bound 5-second implementation." 14
  exit $?
fi
node_before_image="$(docker inspect --format '{{.Image}}' "$node_before")"
if [[ ! "$node_before_image" =~ ^sha256:[0-9a-f]{64}$ ]]; then
  fail "Running Node API has an invalid image identity." 14
  exit $?
fi
...
PY
  ...
  if ((${#receipt_fields[@]} != 4)); then
    fail "Existing pending-cutover Node API journal returned an incomplete identity." 14
    return $?
  fi
...
if [[ -e "$REPAIR_RECEIPT_FILE" || -L "$REPAIR_RECEIPT_FILE" ]]; then
  ...
  if [[ "$receipt_status" == "applied" ]]; then
    ...
    [[ "$(docker exec "$node_before" sha256sum /app/apps/api/src/admin-routes.ts | awk '{print $1}')" == "$EXPECTED_REPAIR_API_SHA256" ]] || {
      fail "Applied repair journal points to a Node API with unexpected source bytes." 14
      exit $?
    }
  ...
  new_image_id="$(docker image inspect --format '{{.Id}}' "$node_image_ref")"
  ...
  docker image tag "$new_image_id" "$repair_image_tag"
