#!/usr/bin/env bash
# Incident-specific recovery for the 2026-07-17 OpenCFD v2606 canary
# retention failure.
#
# The prior same-build replay correctly stopped before attestation when the
# serial RANS canary found a live cases/.../mesh-evidence duplicate after a
# full strip.  This wrapper admits only the reviewed retention correction,
# builds API/worker from an immutable snapshot of that new source, and reuses
# (without rebuilding or retagging) the already-verified Node image.  APP_DIR
# remains on sealed 6338577 until the cutover is terminal and a later normal
# source promotion is explicitly run.
set -Eeuo pipefail
...
EXPECTED_FAILED_REPLAY_SOURCE_REVISION="1f815f89c523cbf667725c1cd681d729c06a10c9"
EXPECTED_FAILED_REPLAY_SOURCE_TREE_SHA256="80b7fd231d15dbf927557c8019c8c55787cac5977b79c00b144cb8007e765bae"
EXPECTED_FAILED_REPLAY_JOURNAL_SHA256="4ca469d5fa5daef9bb43b2e44793890e5cec6a65b60fdf8b415ee0a600e17aec"
EXPECTED_BASE_GIT_TREE_INVENTORY_SHA256="168acd3453ae8472ed841ed390f455902267e27f2c47ba15c182f94f3b52fb01"
EXPECTED_RETENTION_SHA256="f2bd3979d5741d45501e10274eed9abd896020014a9feef88c02a98368540943"
EXPECTED_RETENTION_TEST_SHA256="995eb99157cc172031f32e3968a4c9800a6915cf1d6b596acafe04b641b21fe4"
...
EXPECTED_INITIAL_API_IMAGE="sha256:bc8e23648e9e76424ea36a584f8a825d65fe82a23aa4e4ad89b019197dcc735c"
EXPECTED_INITIAL_WORKER_IMAGE="sha256:42120ef817af19510830d18f99be2b0f8d8739a4b9b235d2ee294e558f64229a"
EXPECTED_INITIAL_ENV_SHA256="c4ff073a4ee698a02cdc70f535f930c93898b69e5f382bf6ac05f965b840acad"
EXPECTED_INITIAL_DATABASE_SHA256="8421c15692afc6e36a834e481734411db61ecf18a95ce4db3a46ec9c7f7ad96c"
EXPECTED_INITIAL_BUILD_ID="prod-20260717-63385777be73-r2"
...
if [[ "${APP_DIR:-$EXPECTED_APP_LINK}" != "$EXPECTED_APP_LINK" || \
  ...
  echo "Pending retention retry refuses caller overrides of bound application, state, lock, or Compose paths." >&2
  exit 2
fi
...
FAILED_REPLAY_JOURNAL_FILE="$AIRFOILS_PRO_STATE_DIR/pending-opencfd2606-rebuild-replay.json"
RETRY_JOURNAL_FILE="$AIRFOILS_PRO_STATE_DIR/pending-opencfd2606-retention-retry.json"
OPENCFD2606_CANARY_RECEIPT_FILE="$AIRFOILS_PRO_STATE_DIR/openfoam-2606-canary-receipt.pending.json"
BASE_INVENTORY_RELATIVE=".codex-artifacts/opencfd2606-retention-base-ls-tree.txt"
NODE_IMAGE_REF="${COMPOSE_PROJECT_NAME}-node-api"
NODE_REPLAY_TAG="airfoils-pro/node-api-cutover-replay:${EXPECTED_FAILED_REPLAY_SOURCE_REVISION:0:12}"
NODE_ROLLBACK_TAG="airfoils-pro/node-api-cutover-replay-rollback:${EXPECTED_FAILED_REPLAY_SOURCE_REVISION:0:12}"
...
running_container() {
  ...
  if [[ -z "$id" || "$(wc -l <<<"$id")" -ne 1 ]]; then
    fail "Retention retry requires exactly one running $service container." 12
    return $?
  fi
  printf '%s' "$id"
}
...
assert_scheduler_stopped_and_idle() {
  if [[ -n "$(compose_bound ps --status running -q sweeper)" ]]; then
    fail "Retention retry requires the scheduler to remain stopped." 12
    return $?
  fi
  ...
  while IFS= read -r service; do
    ...
    [[ -z "$running" ]] || { fail "Retention retry refuses running optional engine worker $service." 12; return $?; }
  ...
  [[ -z "$hidden" ]] || { fail "Retention retry refuses hidden running optional workers: $hidden" 12; return $?; }
  ...
  [[ -z "$output" ]] || { fail "Retention retry refuses active OpenFOAM work: $output" 12; return $?; }
...
persist_retry_journal() {
  ...
  EXPECTED_PRIOR_JOURNAL_SHA="$EXPECTED_FAILED_REPLAY_JOURNAL_SHA256" \
  EXPECTED_NODE_IMAGE_VALUE="$EXPECTED_NODE_IMAGE" EXPECTED_NODE_SOURCE_REVISION_VALUE="$EXPECTED_FAILED_REPLAY_SOURCE_REVISION" \
...
if status == "prepared":
    if os.path.lexists(path): raise SystemExit("refusing to overwrite an existing retention retry journal")
    payload={
      "schemaVersion":1,"purpose":"pending-opencfd2606-retention-retry","status":"prepared",
      "preparedAt":now,"updatedAt":now,"completedAt":None,"failureCount":0,"lastExitCode":None,
      "priorFailedReplayJournalSha256":os.environ["EXPECTED_PRIOR_JOURNAL_SHA"],
      ...
      "priorReplaySourceRevision":os.environ["EXPECTED_NODE_SOURCE_REVISION_VALUE"],
    ...
    if not path.is_file() or path.is_symlink(): raise SystemExit("retention retry journal is missing or unsafe")
...
cleanup() {
  ...
  if ((rc != 0)) && [[ "$journal_armed" == "true" ]]; then
    ...
    persist_retry_journal failed "$rc" false || \
      echo "WARNING: retention retry failed and its last exact runtime binding could not be advanced; the prior journal stage remains authoritative." >&2
    set -e
  fi
...
if [[ ! "$EXPECTED_TARGET_SOURCE_REVISION" =~ ^[0-9a-f]{40}$ ]]; then
  fail "Retention retry requires an exact lowercase target commit SHA." 2; exit $?
fi
if [[ ! -d "$STAGING_DIR" || -L "$STAGING_DIR" || ! -L "$APP_DIR" ]]; then
  fail "Retention retry requires a real staging directory and versioned APP_DIR symlink." 2; exit $?
fi
...
if [[ ! -d "$app_real" || -L "$app_real" || "$app_real" != "$EXPECTED_BOUND_APP_REAL" || "$staging_real" == "$app_real" ]]; then
  fail "Retention retry is not attached to the exact sealed production release." 14; exit $?
fi
if [[ ! -d "$AIRFOILS_PRO_STATE_DIR" || -L "$AIRFOILS_PRO_STATE_DIR" || ! -d /dev/shm || -L /dev/shm ]]; then
  fail "Retention retry requires safe state and in-memory runtime directories." 2; exit $?
fi
...
for required in "$manifest_tool" "$state_tool" "$preflight_tool" "$bound_compose" \
  "$app_real/.deployment-source.json" "$staging_real/.deployment-source.json" "$base_inventory" "$FAILED_REPLAY_JOURNAL_FILE"; do
  [[ -f "$required" && ! -L "$required" ]] || { fail "Retention retry lacks a required safe regular file: $required" 2; exit $?; }
done
require_regular_owned_mode "$ENV_FILE" 600 "Deployment environment"
require_regular_owned_mode "$FAILED_REPLAY_JOURNAL_FILE" 600 "Failed same-build replay journal"
require_regular_owned_mode "$base_inventory" 600 "Authenticated base source inventory"

if [[ "$(sha256_file "$FAILED_REPLAY_JOURNAL_FILE")" != "$EXPECTED_FAILED_REPLAY_JOURNAL_SHA256" ]]; then
  fail "The prior failed replay journal is not the exact incident record authorized for this retry." 14; exit $?
fi
python3 - "$FAILED_REPLAY_JOURNAL_FILE" "$EXPECTED_FAILED_REPLAY_SOURCE_REVISION" \
  "$EXPECTED_FAILED_REPLAY_SOURCE_TREE_SHA256" "$EXPECTED_NODE_IMAGE" "$EXPECTED_INITIAL_API_IMAGE" \
  "$EXPECTED_INITIAL_WORKER_IMAGE" "$EXPECTED_INITIAL_ENV_SHA256" "$EXPECTED_INITIAL_DATABASE_SHA256" <<'PY'
import json,sys
p=json.load(open(sys.argv[1],encoding="utf-8"))
expected={"schemaVersion":1,"purpose":"pending-opencfd2606-same-build-replay","status":"failed",
"failureCount":1,"lastExitCode":14,"replaySourceRevision":sys.argv[2],"replaySourceTreeSha256":sys.argv[3],
"nodeApiImageAfter":sys.argv[4],"currentApiImage":sys.argv[5],"currentWorkerImage":sys.argv[6],
"currentDeploymentEnvironmentSha256":sys.argv[7],"currentDatabaseSnapshotSha256":sys.argv[8],
"currentCanaryReceiptSha256":"absent","currentCutoverStateKind":"pending-pristine"}
for key,value in expected.items():
    if p.get(key)!=value: raise SystemExit(f"failed replay journal mismatch: {key}")
PY
...
if [[ "$bound_revision" != "$EXPECTED_BOUND_SOURCE_REVISION" || "$bound_tree" != "$EXPECTED_BOUND_SOURCE_TREE_SHA256" || "$bound_count" != "$EXPECTED_BOUND_SOURCE_FILE_COUNT" ]]; then
  fail "Retention retry found an unexpected bound release." 14; exit $?
fi
if [[ "$target_revision" != "$EXPECTED_TARGET_SOURCE_REVISION" || "$target_revision" == "$EXPECTED_FAILED_REPLAY_SOURCE_REVISION" ]]; then
  fail "Retention retry target does not match the explicit new dispatch revision." 14; exit $?
fi
target_build_id="prod-20260717-${target_revision:0:12}-r3"

if [[ "$(sha256_file "$base_inventory")" != "$EXPECTED_BASE_GIT_TREE_INVENTORY_SHA256" ]]; then
  fail "The uploaded base Git-tree inventory is not the exact 1f815f8 source inventory." 14; exit $?
fi
python3 - "$manifest_tool" "$base_inventory" "$staging_real" <<'PY'
import hashlib, importlib.util, os, re, stat, sys
from pathlib import Path
tool, inventory_path, target_root=map(Path,sys.argv[1:])
...
for raw in inventory_path.read_text(encoding="utf-8").splitlines():
    ...
    if match is None: raise SystemExit("base Git-tree inventory has an unsupported entry")
    mode,blob,path=match.groups(); relative=Path(path)
    if not module._excluded(relative): base[path]=(mode,blob)
target={}
for path in module._source_entries(target_root):
    ...
    target[relative]=(mode,blob)
changed={path for path in set(base)|set(target) if base.get(path)!=target.get(path)}
expected={"src/airfoilfoam/retention.py","tests/test_retention.py",
          "scripts/deploy/retry-pending-opencfd2606-retention.sh",
          "tests/test_pending_cutover_retention_retry.py"}
if changed != expected:
    print("retention retry source is not the exact reviewed incident scope",file=sys.stderr)
...
for relative in "${!exact_hashes[@]}"; do
  [[ "$(sha256_file "$staging_real/$relative")" == "${exact_hashes[$relative]}" ]] || {
    fail "Retention retry target has unreviewed bytes in $relative." 14; exit $?;
  }
done

runtime_dir="$(mktemp -d /dev/shm/airfoils-opencfd2606-retention.XXXXXX)"
...
if [[ -e "$RETRY_JOURNAL_FILE" || -L "$RETRY_JOURNAL_FILE" ]]; then
  require_regular_owned_mode "$RETRY_JOURNAL_FILE" 600 "Retention retry journal"
  binding="$(python3 - "$RETRY_JOURNAL_FILE" "$target_revision" "$target_tree" "$target_count" "$target_build_id" \
    "$target_application_source_sha" "$EXPECTED_FAILED_REPLAY_JOURNAL_SHA256" "$EXPECTED_NODE_IMAGE" \
    "$EXPECTED_FAILED_REPLAY_SOURCE_REVISION" "$EXPECTED_BOUND_SOURCE_REVISION" \
    "$EXPECTED_BOUND_SOURCE_TREE_SHA256" "$EXPECTED_NODE_ADMIN_ROUTES_SHA256" \
    "$EXPECTED_NODE_ATTESTATION_SHA256" "$EXPECTED_INITIAL_ENV_SHA256" <<'PY'
import json,re,sys
p=json.load(open(sys.argv[1],encoding="utf-8"))
expected={"schemaVersion":1,"purpose":"pending-opencfd2606-retention-retry","engineSourceRevision":sys.argv[2],
"engineSourceTreeSha256":sys.argv[3],"engineSourceFileCount":int(sys.argv[4]),"buildId":sys.argv[5],
"engineApplicationSourceSha256":sys.argv[6],"priorFailedReplayJournalSha256":sys.argv[7],
"nodeApiImage":sys.argv[8],"nodeSourceRevision":sys.argv[9],"priorReplaySourceRevision":sys.argv[9],
"boundReleaseSourceRevision":sys.argv[10],"boundReleaseSourceTreeSha256":sys.argv[11],
"nodeApiAdminRoutesSha256":sys.argv[12],"nodeApiAttestationSha256":sys.argv[13],
"deploymentEnvironmentBeforeSha256":sys.argv[14],"action":"rebuild_and_canary"}
for key,value in expected.items():
    if p.get(key)!=value: raise SystemExit(f"retention retry journal mismatch: {key}")
if p.get("status") not in {"prepared","source_bound","failed","attested_awaiting_continuation","completed"}:
    raise SystemExit("retention retry journal has an invalid status")
allowed=set(expected)|{"status","preparedAt","updatedAt","completedAt","failureCount","lastExitCode",
"nodeApiContainerBefore","nodeApiContainerAfter","sourceBoundDeploymentEnvironmentSha256","promotionEligible",
...
if set(p)!=allowed: raise SystemExit("retention retry journal has unexpected fields")
...
for key in ("sourceBoundDeploymentEnvironmentSha256","currentDeploymentEnvironmentSha256","currentDatabaseSnapshotSha256"):
    if hex64.fullmatch(p.get(key) or "") is None: raise SystemExit(f"retention retry journal has invalid {key}")
for key in ("currentApiImage","currentWorkerImage","currentNodeApiImage"):
    if image.fullmatch(p.get(key) or "") is None: raise SystemExit(f"retention retry journal has invalid {key}")
if p.get("currentNodeApiImage") != p.get("nodeApiImage"):
    raise SystemExit("retention retry journal records a different current Node image")
if p.get("currentNodeExpectedBuildId") not in {"prod-20260717-63385777be73-r2",sys.argv[5]}:
    raise SystemExit("retention retry journal has an unexpected Node build binding")
if p.get("currentCutoverStateKind") not in {"pending-pristine","pending-receipt","pending-unmarked-receipt","pending-attested","pending-attested-retained-receipt","terminal"}:
    raise SystemExit("retention retry journal has an invalid cutover state")
receipt=p.get("currentCanaryReceiptSha256")
if receipt!="absent" and hex64.fullmatch(receipt or "") is None:
    raise SystemExit("retention retry journal has an invalid receipt binding")
if not isinstance(p.get("failureCount"),int) or p["failureCount"]<0:
    raise SystemExit("retention retry journal has an invalid failure count")
if p.get("status")=="completed" and (p.get("currentCutoverStateKind")!="terminal" or p.get("promotionEligible") is not True):
    raise SystemExit("completed retention retry is not terminal/promotion eligible")
if p.get("status")!="completed" and p.get("promotionEligible") is not False:
    raise SystemExit("nonterminal retention retry must not be promotion eligible")
values=[p["status"],p["currentDeploymentEnvironmentSha256"],p["currentApiImage"],p["currentWorkerImage"],
        ...
        p["sourceBoundDeploymentEnvironmentSha256"]]
print("\n".join(map(str,values)))
PY
)" || exit $?
  mapfile -t fields <<<"$binding"
...
if [[ -z "$existing_status" && "$current_build" != "$EXPECTED_INITIAL_BUILD_ID" ]]; then
  fail "Initial retention retry does not have the exact failed-replay build identity." 14; exit $?;
fi
...
if [[ "$existing_status" == "completed" ]]; then
  ...
  echo "Retention retry is already terminal and eligible for a later normal source promotion."
  exit 0
fi
...
case "$state_before" in
  ...
  terminal)
    ...
    echo "Retention retry was already terminal and is now marked promotion eligible."
    exit 0
    ;;
  *) fail "Unsupported cutover state for retention retry: $state_before" 14; exit $? ;;
esac
...
docker exec -e "MAINTENANCE_COOKIE_TTL_MS=$MAINTENANCE_COOKIE_TTL_MS" -w /app "$node_live" pnpm exec tsx -e \
  'import { signSession } from "./apps/api/src/admin-auth.ts"; process.stdout.write("aero_admin=" + signSession("cutover-retention@airfoils.pro", Number(process.env.MAINTENANCE_COOKIE_TTL_MS), "password"));' \
  >"$cookie_file"
...
import json,sys
p=json.load(sys.stdin)
if p.get("authed") is not True or p.get("email") != "cutover-retention@airfoils.pro":
...
[[ -L "$APP_DIR" && "$(readlink -f "$APP_DIR")" == "$EXPECTED_BOUND_APP_REAL" ]] || { fail "APP_DIR moved during retention retry." 14; exit $?; }
snapshot_fields="$(python3 "$snapshot/scripts/deploy/deployment-source-manifest.py" --verify --root "$snapshot" --manifest "$snapshot/.deployment-source.json")"
...
case "$post_state" in
  terminal)
    persist_retry_journal completed "" true
    journal_armed=false
    echo "Retention retry reached terminal cutover proof. A later normal source promotion is now eligible."
    ;;
  pending-attested|pending-attested-retained-receipt)
    persist_retry_journal attested_awaiting_continuation "" false
    journal_armed=false
    echo "Retention retry produced durable canary attestation. Successor evidence is still required; source promotion is NOT eligible."
    ;;
  *)
