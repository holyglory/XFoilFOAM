#!/usr/bin/env bash
# Incident-specific recovery for the 2026-07-17 OpenCFD v2606 attestation
# parser-limit failure.
#
# The exact three-scenario production canary completed and its immutable
# generation-pinned GCS receipt is retained.  Fastify rejected that 2.31 MiB
# receipt before authoritative validation because the route inherited the
# default 1 MiB parser limit.  This wrapper admits only the reviewed,
# route-scoped 4 MiB Node correction, swaps only node-api, and then replays the
# retained receipt through the unchanged certification-only engine script.
# It never rebuilds or recreates api/worker and never reruns a canary.
set -Eeuo pipefail
...
EXPECTED_BASE_SOURCE_REVISION="7a13801aa5b39acba03ff256d1d3256655f6a275"
EXPECTED_BASE_SOURCE_TREE_SHA256="d7583f9a47c33eacee9a099a3721f2a7a5bb506f9ecfcb02201396c47d9e96ed"
EXPECTED_BASE_SOURCE_FILE_COUNT="2208"
EXPECTED_BASE_GIT_INVENTORY_SHA256="08a0dd7c280654ac4567563e773a95420db79286a4ec1be3f4755f978863cb74"
...
EXPECTED_ENV_BEFORE_SHA256="e3a58d52ea4e8e04b305ef084bbf772d626c119978da65d820b7c51c1c8c23bf"
EXPECTED_DATABASE_BEFORE_SHA256="8421c15692afc6e36a834e481734411db61ecf18a95ce4db3a46ec9c7f7ad96c"
EXPECTED_FAILED_JOURNAL_SHA256="075f5dc83dd01356fd6192375d595cf28ad5625fec30499215de4fa235b19236"
...
EXPECTED_NODE_CONTAINER="3bb29e702d06dbe1e6f657b3aa7c94e65164d5c12bd215e60bbfa5ac55a8a864"
...
EXPECTED_BASE_ROUTE_SHA256="8510628cdc4e2e7e625776acbf63fa571c0b0e72d3a16f789ac94757b55e24db"
EXPECTED_BASE_ROUTE_TEST_SHA256="406ec040921097d49f1ac2ce52197e3e45eed86dd6277703af2043afe3d8b1b4"
EXPECTED_TARGET_ROUTE_SHA256="806243c45225487036f98bf144f6bc362e0d54324f640ac59a440f5a5700d15d"
EXPECTED_TARGET_ROUTE_TEST_SHA256="ea1d72664200d4c29f140b0ca543e34c2bdb5e35c58e61081aa7a5cfd8afd6bd"
...
FAILED_JOURNAL_FILE="$AIRFOILS_PRO_STATE_DIR/pending-opencfd2606-early-stop-retention-retry.json"
RECOVERY_JOURNAL_FILE="$AIRFOILS_PRO_STATE_DIR/pending-opencfd2606-attestation-413-recovery.json"
OPENCFD2606_CANARY_RECEIPT_FILE="$AIRFOILS_PRO_STATE_DIR/openfoam-2606-canary-receipt.pending.json"
BASE_INVENTORY_RELATIVE=".codex-artifacts/opencfd2606-attestation-413-base-ls-tree.txt"
TARGET_INVENTORY_RELATIVE=".codex-artifacts/opencfd2606-attestation-413-target-ls-tree.txt"
NODE_IMAGE_REF="${COMPOSE_PROJECT_NAME}-node-api"
NODE_REPAIR_TAG="airfoils-pro/node-api-attestation-413:${EXPECTED_TARGET_SOURCE_REVISION:0:12}"
NODE_ROLLBACK_TAG="airfoils-pro/node-api-attestation-413-rollback:${EXPECTED_BASE_SOURCE_REVISION:0:12}"
...
node_source_hashes() {
  ...
  docker exec "$id" sh -ec \
    'sha256sum /app/apps/api/src/engine-cutover-routes.ts /app/apps/api/test/engine-cutover-routes.test.ts' \
    | awk '{print $1}'
}

assert_node_source() {
  local expected_route="$1" expected_test="$2" hashes id admin_sha attestation_sha baked_build
  id="$(running_container node-api)" || return $?
  hashes="$(node_source_hashes)" || return $?
  [[ "$hashes" == "$expected_route"$'\n'"$expected_test" ]] || {
    fail "Live Node API does not contain the exact reviewed route and regression bytes." 12
    return $?
  }
...
if p != {"ok":True,"service":"aerodb-api"}: raise SystemExit("Node API health payload is invalid")
  ...
  [[ "$admin_sha" == "$EXPECTED_NODE_ADMIN_ROUTES_SHA256" && \
     "$attestation_sha" == "$EXPECTED_NODE_ATTESTATION_SHA256" && \
...
assert_failed_journal() {
  require_regular_owned_mode "$FAILED_JOURNAL_FILE" 600 "Failed early-stop retry journal"
  [[ "$(sha256_file "$FAILED_JOURNAL_FILE")" == "$EXPECTED_FAILED_JOURNAL_SHA256" ]] || {
    fail "Failed retry journal differs from the exact attestation-413 incident." 14
    return $?
  }
  python3 - "$FAILED_JOURNAL_FILE" <<'PY'
import json,sys
p=json.load(open(sys.argv[1],encoding="utf-8"))
expected={"schemaVersion":1,"purpose":"pending-opencfd2606-early-stop-retention-retry",
...
persist_journal() {
  ...
  temp="$(mktemp "$AIRFOILS_PRO_STATE_DIR/.pending-opencfd2606-attestation-413.XXXXXX")"
  chmod 600 "$temp"
...
p={
 "schemaVersion":1,"purpose":"pending-opencfd2606-attestation-413-recovery",
 ...
 "baseSourceRevision":"7a13801aa5b39acba03ff256d1d3256655f6a275",
 "baseSourceTreeSha256":"d7583f9a47c33eacee9a099a3721f2a7a5bb506f9ecfcb02201396c47d9e96ed",
 ...
 "routeSha256":"806243c45225487036f98bf144f6bc362e0d54324f640ac59a440f5a5700d15d",
 "routeTestSha256":"ea1d72664200d4c29f140b0ca543e34c2bdb5e35c58e61081aa7a5cfd8afd6bd",
 "failedRetryJournalSha256":"075f5dc83dd01356fd6192375d595cf28ad5625fec30499215de4fa235b19236",
 ...
 "deploymentEnvironmentBeforeSha256":"e3a58d52ea4e8e04b305ef084bbf772d626c119978da65d820b7c51c1c8c23bf",
 ...
 "nodeApiImageBefore":"sha256:89069188e2e57cad231dcf3527eaba2e5151b0886921eac4f720d589d09e66af",
 "nodeApiContainerBefore":"3bb29e702d06dbe1e6f657b3aa7c94e65164d5c12bd215e60bbfa5ac55a8a864",
...
if p.get("promotionEligible") is not False or p.get("completedAt") is not None:
    raise SystemExit("413 recovery journal must remain nonterminal and promotion-ineligible")
...
fd,name=tempfile.mkstemp(prefix=f".{path.name}.attestation-413-",dir=path.parent)
try:
...
rollback_node() {
  echo "Rolling back the uncommitted Node-only repair..."
  ...
  assert_node_source "$EXPECTED_BASE_ROUTE_SHA256" "$EXPECTED_BASE_ROUTE_TEST_SHA256"
}
...
if [[ ! "$EXPECTED_TARGET_SOURCE_REVISION" =~ ^[0-9a-f]{40}$ || \
  ...
  fail "Attestation recovery requires an explicit distinct target commit and inventory." 2
  exit $?
fi
if [[ ! -d "$STAGING_DIR" || -L "$STAGING_DIR" || ! -L "$APP_DIR" ]]; then
  fail "Attestation recovery requires a real staging directory and versioned APP_DIR symlink." 2
  exit $?
fi
...
if [[ "$app_real" != "$EXPECTED_BOUND_APP_REAL" || ! -d "$app_real" || -L "$app_real" || "$staging_real" == "$app_real" ]]; then
  fail "Attestation recovery is not attached to the exact sealed production release." 14
  exit $?
fi
if [[ ! -d "$AIRFOILS_PRO_STATE_DIR" || -L "$AIRFOILS_PRO_STATE_DIR" || ! -d /dev/shm || -L /dev/shm ]]; then
  fail "Attestation recovery requires safe state and in-memory runtime directories." 2
  exit $?
fi
...
for required in "$manifest_tool" "$state_tool" "$preflight_tool" \
  ...
  [[ -f "$required" && ! -L "$required" ]] || {
    fail "Attestation recovery lacks required safe source evidence: $required" 2
    exit $?
  }
done
...
scaffolding={"scripts/deploy/recover-pending-opencfd2606-attestation-413.sh",
             "tests/test_pending_cutover_attestation_413_recovery.py"}
expected={"apps/api/src/engine-cutover-routes.ts",
          "apps/api/test/engine-cutover-routes.test.ts"}|scaffolding
if changed!=expected: raise SystemExit("target is not the exact reviewed 413 recovery delta")
if any(p in base or p not in target for p in scaffolding):
    raise SystemExit("413 recovery scaffolding is not additive")
...
[[ "$(sha256_file "$staging_real/apps/api/src/engine-cutover-routes.ts")" == "$EXPECTED_TARGET_ROUTE_SHA256" && \
   "$(sha256_file "$staging_real/apps/api/test/engine-cutover-routes.test.ts")" == "$EXPECTED_TARGET_ROUTE_TEST_SHA256" && \
  ...
  fail "Target has changed reviewed route or immutable certification bytes." 14; exit $?
}

runtime_dir="$(mktemp -d /dev/shm/airfoils-attestation-413.XXXXXX)"
...
if [[ "$current_node_hashes" == "$EXPECTED_TARGET_ROUTE_SHA256"$'\n'"$EXPECTED_TARGET_ROUTE_TEST_SHA256" ]]; then
  node_image_after="$(service_image node-api)"
  node_container_after="$(running_container node-api)"
  assert_node_source "$EXPECTED_TARGET_ROUTE_SHA256" "$EXPECTED_TARGET_ROUTE_TEST_SHA256"
  if [[ "$journal_phase" == "prepared" || "$journal_phase" == "failed" ]]; then
    persist_journal node_applied "" false
    journal_phase="node_applied"
  fi
elif [[ "$current_node_hashes" == "$EXPECTED_BASE_ROUTE_SHA256"$'\n'"$EXPECTED_BASE_ROUTE_TEST_SHA256" ]]; then
  ...
  node_container_after="$(running_container node-api)"
  if ! assert_node_source "$EXPECTED_TARGET_ROUTE_SHA256" "$EXPECTED_TARGET_ROUTE_TEST_SHA256" || \
...
docker exec -e "MAINTENANCE_COOKIE_TTL_MS=$MAINTENANCE_COOKIE_TTL_MS" -w /app "$node_live" \
  pnpm exec tsx -e \
  'import { signSession } from "./apps/api/src/admin-auth.ts"; process.stdout.write("aero_admin=" + signSession("cutover-attestation-413@airfoils.pro", Number(process.env.MAINTENANCE_COOKIE_TTL_MS), "password"));' \
  >"$cookie_file"
...
import json,sys
p=json.load(sys.stdin)
if p.get("authed") is not True or p.get("email")!="cutover-attestation-413@airfoils.pro":
