...
from thermally_perfect_shock import enthalpy_change, entropy_change_over_R, weak_thermally_perfect_shock
...
def timestep_courant(physical_time, target_courant):
    if target_courant is not None and (not physical_time or isinstance(target_courant, bool)
            or not math.isfinite(target_courant) or not 0 < target_courant <= 0.49):
        raise ValueError("A timestep study requires physical time and a positive Courant target at most0.49")
    return (0.49 if target_courant is None else target_courant) if physical_time else 0.5


def write_case(directory, cells, recipe, mach=REFERENCE_MACH, physical_time=False, gas=None, target_courant=None):
    ...
    courant = timestep_courant(physical_time, target_courant)
    reference = weak_oblique_shock(mach, REFERENCE_ANGLE, REFERENCE_GAMMA) if gas is None else weak_thermally_perfect_shock(gas, mach, REFERENCE_ANGLE, REFERENCE_TEMPERATURE)
    speed = mach * (math.sqrt(REFERENCE_GAMMA * REFERENCE_GAS_CONSTANT * REFERENCE_TEMPERATURE) if gas is None
                    else gas.speed_of_sound(ThermodynamicState(temperature_k=REFERENCE_TEMPERATURE, pressure_pa=REFERENCE_PRESSURE)))
...
frontAndBack { type empty; faces ((0 3 2 1) (4 5 6 7) (1 2 9 8) (5 10 11 6)); }
    ...
    write_foam_dict(directory / "system/controlDict", "dictionary", "controlDict", {
        ...
        "adjustTimeStep": "yes" if physical_time else "no", "maxCo": courant, "maxDeltaT": length / speed,
        ...
        **({"libs": [Raw('"/opt/xfoilfoam-thermophysics/lib/libxfoilfoamThermophysics.so"')]}
           if gas is not None and gas.transport_model == "polynomial" else {}),
    })
    ...
    if gas is not None:
        write_foam_dict(directory / "constant/thermophysicalProperties", "dictionary", "thermophysicalProperties", gas.openfoam_dictionary())
...
def conservation_errors(pressure_ratio, density_ratio, temperature_ratio, velocity, reference, gas=None):
    ...
    gas_constant = REFERENCE_GAS_CONSTANT if gas is None else gas.gas_constant
    upstream_density = REFERENCE_PRESSURE / (gas_constant * REFERENCE_TEMPERATURE)
    upstream_speed = reference.mach_upstream * math.sqrt(reference.gamma * gas_constant * REFERENCE_TEMPERATURE)
    ...
    heat_capacity = reference.gamma * gas_constant / (reference.gamma - 1)
    temperature_downstream = REFERENCE_TEMPERATURE * temperature_ratio
    enthalpy_difference = heat_capacity * (temperature_downstream - REFERENCE_TEMPERATURE) if gas is None else enthalpy_change(gas, REFERENCE_TEMPERATURE, temperature_downstream)
    return {
        ...
        "total_energy": abs(enthalpy_difference + (sum(value ** 2 for value in velocity) - upstream_speed ** 2) / 2)
                            / (heat_capacity * REFERENCE_TEMPERATURE + upstream_speed ** 2 / 2),
        ...
        "entropy_change_over_R": (math.log(pressure_ratio) - reference.gamma * math.log(density_ratio)) / (reference.gamma - 1)
        if gas is None else entropy_change_over_R(gas, REFERENCE_TEMPERATURE, temperature_downstream, pressure_ratio),
    }


def analyze_case(directory, reference, coordinate_kind="iteration", gas=None):
    ...
    for probe in [1, 2, 3]:
        ...
        gamma = REFERENCE_GAMMA if gas is None else gas.gamma_at(means["T"][probe])
        observed["mach_downstream"].append(velocity / math.sqrt(gamma * gas_constant * means["T"][probe]))
        conservation.append(conservation_errors(observed["pressure_ratio"][-1], observed["density_ratio"][-1],
            observed["temperature_ratio"][-1], means["U"][3 * probe:3 * probe + 3], reference, gas))
    ...
    scales = {"p": REFERENCE_PRESSURE, "rho": upstream_density, "T": REFERENCE_TEMPERATURE,
              "U": reference.mach_upstream * math.sqrt(reference.gamma * gas_constant * REFERENCE_TEMPERATURE)}
    variations = {}
    ...
    return {**comparison, "conservation_version": "mass-momentum-enthalpy-v2", "observed": observed, "pressure_window_variation": stability, "upstream_pressure_error": upstream_error,
...
def main():
    ...
    parser.add_argument("--target-courant", type=float)
    args = parser.parse_args()
    try:
        courant = timestep_courant(args.physical_time, args.target_courant)
    ...
    report = {"kind": "inviscid-oblique-shock-verification", "source": NASA_WEDGE_REFERENCE,
              ...
              "target_courant": courant, "measured_courant_ceiling": 0.5,
              "airfoil_polar_validation": False, "outcome": "failed"}
    try:
        gas = None
        if args.material is not None:
            from airfoilfoam.numerical_canary import source_material_for_canary
            gas = source_material_for_canary(args.material)
            report.update({"kind": "thermally-perfect-shock-verification", "reference_kind": "conservation_equations_with_declared_material",
                           "material_sha256": hashlib.sha256(args.material.read_bytes()).hexdigest(), "material": gas.model_dump(mode="json"),
                           "source_reference": "https://www.grc.nasa.gov/www/k-12/airplane/normal.html"})
        reference, probes = write_case(directory, args.cells, args.recipe, args.mach, args.physical_time, gas, args.target_courant)
        ...
        report["comparison"] = analyze_case(directory, reference, report["coordinate_kind"], gas)
