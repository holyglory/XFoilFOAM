...
def diagnostic_source_identity(source_root, loaded_package):
    root = Path(source_root).resolve()
    if not (root / "pyproject.toml").is_file() or Path(loaded_package).resolve() != root / "src/airfoilfoam/__init__.py":
        raise ValueError("The diagnostic requires the complete actually loaded source manifest")
    return {"engine_application_source_sha256": application_source_sha256(root),
            "engine_source_revision": None, "build_id": "isolated-mach3-diagnosis"}


def native_image_fingerprints(root=Path("/etc")):
    fingerprints = {}
    for field, name in (("engine_package_sha256", "package"), ("engine_binary_sha256", "binary")):
        value = (root / f"airfoilfoam-engine-{name}-sha256").read_text().strip()
        if not re.fullmatch(r"[0-9a-f]{64}", value):
            raise ValueError("Invalid native image fingerprint")
...
def summarize_outcomes(result):
    return [{"aoa_deg": point.aoa_deg, "converged": point.converged, "error": point.error,
             "iterations": point.iterations, "final_residual": point.final_residual,
...
def authenticate_archives(job_dir):
    proofs = []
    for archive in sorted(Path(job_dir).rglob("engine_evidence.tar.zst")):
        manifest = (archive.parent / "evidence_manifest.json").read_bytes()
        record, members = verify_local_archive_manifest_members(archive, expected_manifest=manifest)
        proofs.append({"archive": archive.relative_to(job_dir).as_posix(),
                       "archive_sha256": record.stored_sha256, "archive_bytes": record.stored_size,
                       "manifest_sha256": hashlib.sha256(manifest).hexdigest(), "authenticated_members": members})
    return proofs
