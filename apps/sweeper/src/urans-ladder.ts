...
async function consumeUransRequest(
  ...
  if (requestedFidelity === "precalc" && request.continueFromResultId) {
    ...
    if (incompatibleSource) {
      ...
      // A reducer-owned archive continuation has an immutable recovery action
      // as well as this request receipt.  Do not leave that action looking
      // successfully routed after the normal consumer rejects the source: it
      // would no longer be claimable and the exact archive repair would be
      // silently stranded.  Ordinary/manual continuations retain their
      // existing cancellation behavior.
      if (archiveBackfillRequiresActionProof) {
        await blockArchiveBackfillPrecalcContinuationAtSubmit(db, {
          requestId: request.id,
          reason:
            "the archive-pinned continuation source no longer matches the target solver implementation, boundary, or physical cell",
        });
      } else {
        await db
          .update(simUransRequests)
