...
export type GuardedSubmitOutcome =
  ...
  | { kind: "storage_admission_hold"; error: string }
  | {
...
export async function submitPendingJobWithLifecycleGuard(opts: {
  ...
  /**
   * Optional last-moment storage permit owned by the scheduler. It runs only
   * after this exact job has acquired `engine_state=submitting`, so the disk
   * exposure query includes the candidate's real fidelity and case count
   * rather than the generic next-job allowance. A denied/failed check releases
   * the unexecuted composition without consuming solver or ladder attempts.
   */
  ...
  if (opts.storageAdmissionAllowed) {
    let storageAllowed = false;
    let storageError: string | null = null;
    try {
      storageAllowed = await opts.storageAdmissionAllowed();
    } catch (error) {
      storageError = errorMessage(error);
    }
    if (!storageAllowed) {
      const reason = storageError
        ? `storage admission check failed after exact job reservation: ${storageError}`
        : "storage admission closed after exact job reservation";
      ...
      return { kind: "storage_admission_hold", error: reason };
    }
  }
