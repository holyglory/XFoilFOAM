...
function own<T extends { jobId: string } | string>(calls: T[]): T[] {
  return calls.filter((c) => (typeof c === "string" ? c : c.jobId).startsWith(PREFIX));
}

function fakeEngine(opts: {
  strip?: (jobId: string, keepCaseState: boolean | undefined) => Promise<{ bytes_freed: number; files_removed: number; kept_case_state: boolean }>;
  maintenanceJobs?: { items: { job_id: string; mtime_epoch: number; bytes: number | null }[] };
  ...
  return {
    stripCalls,
    deleteCalls,
    stripJob: async (jobId: string, request: { keep_case_state?: boolean } = {}) => {
    ...
    maintenanceDisk: async () => opts.disk ?? { total_bytes: 1000, free_bytes: 250, used_pct: 75 },
  ...
  } as unknown as EngineClient & { stripCalls: StripCall[]; deleteCalls: string[] };
}
...
async function insertTerminalJob(engineJobId: string, patch: Partial<typeof simJobs.$inferInsert> = {}) {
  const [job] = await db
    .insert(simJobs)
    .values({
      airfoilId,
...
async function readJob(id: string) {
  const [row] = await db.select().from(simJobs).where(eq(simJobs.id, id)).limit(1);
...
beforeAll(async () => {
  ...
  const [condition] = await db.select().from(simCampaignConditions).where(eq(simCampaignConditions.campaignId, campaignId)).limit(1);
...
afterAll(async () => {
  if (revisionId) await db.delete(simUransRequests).where(eq(simUransRequests.revisionId, revisionId));
  await cleanupCampaignFixtures(db, {
    campaignIds: [campaignId],
    presetSlugPrefix: `campaign-${PREFIX.toLowerCase()}`,
  });
  const campaignFlowIds = await db.select({ id: flowConditions.id }).from(flowConditions).where(like(flowConditions.slug, `${PREFIX}%`));
  if (campaignFlowIds.length) await db.delete(flowConditions).where(inArray(flowConditions.id, campaignFlowIds.map((r) => r.id)));
  ...
  if (campaignGeoIds.length) await db.delete(referenceGeometryProfiles).where(inArray(referenceGeometryProfiles.id, campaignGeoIds.map((r) => r.id)));
...
describe("retention schema", () => {
  it("pins migration 0039 sim_jobs strip columns", async () => {
    ...
    expect(cols.map((c) => c.column_name).sort()).toEqual(["strip_report", "stripped_at"]);
  });
});
...
describe("terminal strip reaper", () => {
  it("strips terminal accepted jobs without keeping case state and persists the report", async () => {
    ...
    await insertClassifiedResult(job, { state: "accepted", fidelity: "urans_full" });
    const engine = fakeEngine();

    await stripTerminalJobs(db, engine, { now: NOW, stripMinAgeMs: THIRTY_MIN, stripMaxPerTick: 500 });

    expect(own(engine.stripCalls)).toEqual([{ jobId: `${PREFIX}-accepted-strip`, keepCaseState: false }]);
    const after = await readJob(job.id);
    expect(after.strippedAt?.toISOString()).toBe(NOW.toISOString());
    expect(after.stripReport).toMatchObject({ bytes_freed: 2048, files_removed: 2, kept_case_state: false });
  });
  ...
  it("keeps case state for a budget-stop continuable row, then fully strips after supersession", async () => {
    ...
    const row = await insertClassifiedResult(job, {
      state: "rejected",
      qualityWarnings: [`URANS integration ${URANS_BUDGET_STOP_MARKER}: retained 1.2 periods`],
    });
    const engine = fakeEngine();

    await stripTerminalJobs(db, engine, { now: NOW, stripMinAgeMs: THIRTY_MIN, stripMaxPerTick: 500 });
    expect(own(engine.stripCalls)).toEqual([{ jobId: `${PREFIX}-continuable-strip`, keepCaseState: true }]);
    ...
    await stripTerminalJobs(db, engine, { now: new Date(NOW.getTime() + 60_000), stripMinAgeMs: THIRTY_MIN, stripMaxPerTick: 500 });
  ...
  it("fully strips an expired continuable-kept job even while the continuable row remains", async () => {
    ...
    const job = await insertTerminalJob(`${PREFIX}-continuable-expired`, {
      ...
      stripReport: { bytes_freed: 123, files_removed: 4, kept_case_state: true },
    });
    ...
    await stripTerminalJobs(db, engine, { now: NOW, stripMinAgeMs: THIRTY_MIN, retentionContinuableDays: 14, stripMaxPerTick: 500 });

    expect(own(engine.stripCalls)).toEqual([{ jobId: `${PREFIX}-continuable-expired`, keepCaseState: false }]);
    const after = await readJob(job.id);
    expect(after.stripReport).toMatchObject({ kept_case_state: false });
  });
  ...
  it("keeps case state when a live continuation request points at a result of the job", async () => {
    ...
    const source = await insertClassifiedResult(job, { state: "accepted", fidelity: "urans_full" });
    ...
    expect(own(engine.stripCalls)).toEqual([{ jobId: `${PREFIX}-live-continuation`, keepCaseState: true }]);
  });
  ...
  it("leaves 409 jobs unstamped for retry and stamps 404 jobs as no-op", async () => {
    ...
    const conflictEngine = fakeEngine({
      strip: async (_jobId, keepCaseState) => {
        ...
        return { bytes_freed: 8192, files_removed: 3, kept_case_state: Boolean(keepCaseState) };
      },
    });

    await stripTerminalJobs(db, conflictEngine, { now: NOW, stripMinAgeMs: THIRTY_MIN, stripMaxPerTick: 500 });
    expect((await readJob(conflictJob.id)).strippedAt).toBeNull();

    await stripTerminalJobs(db, conflictEngine, { now: new Date(NOW.getTime() + 60_000), stripMinAgeMs: THIRTY_MIN, stripMaxPerTick: 500 });
    expect(own(conflictEngine.stripCalls).map((c) => c.jobId)).toEqual([`${PREFIX}-strip-409`, `${PREFIX}-strip-409`]);
    expect((await readJob(conflictJob.id)).stripReport).toMatchObject({ bytes_freed: 8192 });
    ...
    await stripTerminalJobs(db, missingEngine, { now: NOW, stripMinAgeMs: THIRTY_MIN, stripMaxPerTick: 500 });
    const missingAfter = await readJob(missingJob.id);
    expect(missingAfter.strippedAt?.toISOString()).toBe(NOW.toISOString());
    expect(missingAfter.stripReport).toMatchObject({ bytes_freed: 0, files_removed: 0, kept_case_state: false, note: "engine job not found" });
  });

  it("never strips running, pending, or too-young jobs", async () => {
    const running = await insertTerminalJob(`${PREFIX}-running-never`, { status: "running" });
    const pending = await insertTerminalJob(`${PREFIX}-pending-never`, { status: "pending" });
    const young = await insertTerminalJob(`${PREFIX}-young-never`, { finishedAt: new Date(NOW.getTime() - 1_000), ingestedAt: new Date(NOW.getTime() - 1_000) });
...
describe("orphan sweep", () => {
  it("deletes old unknown engine dirs, keeps young unknown dirs, and never deletes known sim_jobs dirs", async () => {
    ...
    const engine = fakeEngine({
      disk: { total_bytes: 100 * 1024 ** 3, free_bytes: 15 * 1024 ** 3, used_pct: 85 },
      maintenanceJobs: {
        items: [
          { job_id: `${PREFIX}-old-unknown`, mtime_epoch: (NOW.getTime() - 72 * 60 * 60 * 1000) / 1000, bytes: 10_000 },
          { job_id: `${PREFIX}-young-unknown`, mtime_epoch: (NOW.getTime() - 60 * 60 * 1000) / 1000, bytes: 20_000 },
          { job_id: `${PREFIX}-known-engine-dir`, mtime_epoch: (NOW.getTime() - 72 * 60 * 60 * 1000) / 1000, bytes: 30_000 },
        ],
      },
      deleteBytes: { [`${PREFIX}-old-unknown`]: 5000 },
    });

    const deleted = await runOrphanSweep(db, engine, { now: NOW, orphanMinAgeMs: 48 * 60 * 60 * 1000 });
