...
describe("repeated preliminary URANS quality recovery", () => {
  ...
  it.each([null, 0, 11, 12, 13])(
    "defers a repeated attempt on pre-v14 engine capability %p",
    (version) => {
  ...
  it("pins v14 and selects recovery in both engine and durable job payloads", () => {
    const request = {
      solver: {
    ...
    const plan = conservativeUransRetryPlan(true, 14);
    expect(plan).toEqual({ kind: "required", recoveryVersion: 14 });
    ...
    expect(request.expected_urans_recovery_version).toBe(14);
    expect({
      uransQualityRecovery: true,
      uransRecoveryVersion: plan.recoveryVersion,
    }).toEqual({ uransQualityRecovery: true, uransRecoveryVersion: 14 });
  });
});
