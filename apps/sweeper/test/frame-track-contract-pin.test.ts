...
import { classifyPolarEvidence } from "@aerodb/core";
...
describe("frame-track contract pin (fixture JSON)", () => {
  ...
  it("rejects an ADDED top-level key (engine grew the payload silently)", () => {
    ...
    expect(parsed.errors.join(" ")).toContain(
      'unexpected key "shedding_freq_hz"',
    );
  });
  ...
  it("rejects a RENAMED key (periods_retained → periodsRetained)", () => {
    ...
    expect(parsed.errors.join(" ")).toContain(
      'unexpected key "periodsRetained"',
    );
  });
  ...
  it("rejects a RETYPED key (stationary as string)", () => {
    ...
    if (parsed.ok) return;
    expect(parsed.errors.join(" ")).toContain(
      "frame_track.stationary: expected boolean",
    );
  });
  ...
  it("rejects a frames array beyond the pinned <=120 cap", () => {
    ...
    drifted.frames = Array.from(
      { length: FRAME_TRACK_MAX_FRAMES + 1 },
      (_, i) => ({ ...frame, i, t: 10 + i * 0.005 }),
    );
    const parsed = parseFrameTrack(drifted);
    expect(parsed.ok).toBe(false);
    if (parsed.ok) return;
  ...
  it("pins the frame image artifact kind and image_pattern rendering", () => {
    ...
    expect(
      frameImageRelativePath("frames/{field}/f{i04}.png", "vorticity", 7),
    ).toBe("frames/vorticity/f0007.png");
    expect(
      frameImageRelativePath(
        "frames/{field}/f{i04}.png",
        "velocity_magnitude",
        119,
      ),
    ).toBe("frames/velocity_magnitude/f0119.png");
  });
  ...
  it("pins the PRODUCTION multi-AoA image_pattern (a{i}/ case namespace)", () => {
    ...
    const prefixed = {
      ...contractFixture(),
      image_pattern: "a2/frames/{field}/f{i04}.png",
    };
    ...
    expect(
      frameImageRelativePath("a2/frames/{field}/f{i04}.png", "vorticity", 7),
    ).toBe("a2/frames/vorticity/f0007.png");
  });
});
...
describe("ingest frame_track persistence (frameTrackForPoint)", () => {
  ...
  it("persists NULL for steady/no-shedding points (unsteady=false, frame_track null/absent)", () => {
    ...
    expect(
      frameTrackForPoint({ ...steady, frame_track: null }, "test"),
    ).toBeNull();
  ...
  it("persists a FAIL-CLOSED sentinel for a SHEDDING point missing frame_track (never null)", () => {
    ...
    for (const point of [{ ...basePoint, frame_track: null }, basePoint]) {
      const persisted = frameTrackForPoint(
        point,
        "job j1, case c1, aoa 16",
      ...
      const classified = classifyPolarEvidence([
        {
          id: "fresh-precalc-missing-frame-track",
          a: 16,
          cl: 1.0,
          cd: 0.1,
          cm: -0.02,
  ...
  it("persists a contract-valid frame_track verbatim without warnings", () => {
    ...
    const persisted = frameTrackForPoint(
      { ...basePoint, frame_track: fixture as never },
      "test",
    );
  ...
  it("persists a DRIFTED frame_track verbatim but logs the contract drift loudly", () => {
    ...
    const persisted = frameTrackForPoint(
      { ...basePoint, frame_track: drifted as never },
      "job j1, case c1, aoa 16",
    );
