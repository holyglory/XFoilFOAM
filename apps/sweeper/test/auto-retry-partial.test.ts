...
// keep marching — so the point terminalized (failed / needs_review) through
...
//      partial → the one-shot requeue fires immediately (pending + marker +
//      campaign point requested + loud log) and the gap finder re-picks.
...
//      escalates: row stays failed, point terminal, needs_review counts it.
...
import type { EngineClient, JobResult, JobStatus, PolarPoint, PolarRequest } from "@aerodb/engine-client";
...
const DIVERGENCE_MESSAGE = "transient diverged at t=1e-05: |Cl|=7.07e4, dt=1e-07";
...
function runningPartialEngine(engineJobId: string, speed: number, points: PolarPoint[], totalCases: number): EngineClient {
  return {
    getQueue: async () => {
    ...
    getResult: async (): Promise<JobResult> => ({
      job_id: engineJobId,
      state: "running",
      polars: [{ speed, chord: CHORD, reynolds: reynoldsOf(speed), mach: speed / 340.3, points }],
    }),
  } as unknown as EngineClient;
}

function terminalEngine(engineJobId: string, state: "completed" | "failed", speed: number, points: PolarPoint[], message?: string): EngineClient {
  return {
    getQueue: async () => {
      throw new Error("queue unavailable in test");
    },
...
async function composeCampaignJob(campaignId: string, engineJobId: string): Promise<{ jobId: string; entries: ConditionMapEntry[]; airfoilId: string }> {
...
async function cellRows(revisionId: string, forAirfoilId = airfoilId) {
  return db
    ...
    .from(results)
    .where(and(eq(results.airfoilId, forAirfoilId), eq(results.simulationPresetRevisionId, revisionId)))
    .orderBy(asc(results.aoaDeg));
}

async function pointStates(campaignId: string, forAirfoilId: string) {
  return db
    .select({ aoaDeg: simCampaignPoints.aoaDeg, state: simCampaignPoints.state })
    .from(simCampaignPoints)
    .where(and(eq(simCampaignPoints.campaignId, campaignId), eq(simCampaignPoints.airfoilId, forAirfoilId)))
...
beforeAll(async () => {
  ...
  const [airfoil] = await db
    .insert(airfoils)
    .values({ slug: `${PREFIX}-cambered-a`, name: `${PREFIX} cambered a`, categoryId: cat.id, points: camberedPoints, isSymmetric: false })
    .returning();
  airfoilId = airfoil.id;
  const [airfoil2] = await db
    .insert(airfoils)
    .values({ slug: `${PREFIX}-cambered-b`, name: `${PREFIX} cambered b`, categoryId: cat.id, points: camberedPoints, isSymmetric: false })
    .returning();
...
describe("auto-retry-once on the running-partial ingest route (live gap 2026-07-08)", () => {
  it("MUST-CATCH: a precalc wave-2 child's diverged point in a RUNNING partial auto-retries immediately (marker + requested + loud log + gap finder re-pick)", async () => {
    const errSpy = vi.spyOn(console, "error");
    const composed = await composeCampaignJob(campaignAId, `${PREFIX}-engine-w1`);
    ...
    const [child] = await db
      ...
      .values({
        ...
        requestPayload: {
          speedMap: [{ speed: SPEED_A, bcId: bcAId, presetRevisionId: revisionAId, mach: SPEED_A / 340.3 }],
          aoas: ANGLES,
          parentJobId: wave1JobId,
    ...
    childJobId = child.id;
    await db.update(results).set({ simJobId: childJobId }).where(eq(results.simJobId, wave1JobId));
    await db.update(simJobs).set({ status: "done", ingestedAt: new Date(), finishedAt: new Date() }).where(eq(simJobs.id, wave1JobId));

    // The divergence watchdog killed the −5° case at t≈1e-05; the job is
    // STILL RUNNING its other angles when the sweeper polls.
    await reconcile(db, runningPartialEngine(`${PREFIX}-engine-w2`, SPEED_A, [divergedPrecalcPoint(-5)], ANGLES.length), {
      jobIds: [childJobId],
    ...
    expect(crashed.status).toBe("pending"); // re-claimable, NOT failed
    ...
    expect(points.find((p) => Number(p.aoaDeg) === -5)?.state).toBe("requested"); // reopened
    ...
    const [childRow] = await db.select({ status: simJobs.status }).from(simJobs).where(eq(simJobs.id, childJobId));
    expect(childRow.status).toBe("running");

    // Gap finder re-picks the released cell on the next tick.
    const rePick = await findCampaignGapBatch(db, { limit: 500, campaignIds: [campaignAId] });
    expect(rePick).not.toBeNull();
    expect(rePick!.angles).toContain(-5);

    // Loud by contract: the requeue announces itself.
    expect(errSpy.mock.calls.some((call) => String(call[0]).includes("AUTO-RETRY: crash-class failed point requeued ONCE"))).toBe(true);
  }, 240000);
  ...
  it("MUST-CATCH: the same job's terminal failed ingest re-ships the diverged case — released cell stays released (no false escalation), other claimed rows get their own first retry", async () => {
    ...
    await reconcile(db, terminalEngine(`${PREFIX}-engine-w2`, "failed", SPEED_A, [divergedPrecalcPoint(-5)], CRASH_MESSAGE), {
      jobIds: [childJobId],
      skipFailedRecovery: true,
    });
    ...
    expect(errSpy.mock.calls.some((call) => String(call[0]).includes("RELEASED-CELL GUARD"))).toBe(true);
    // One crash must not consume both the retry and the escalation.
    expect(errSpy.mock.calls.some((call) => String(call[0]).includes("AUTO-RETRY EXHAUSTED"))).toBe(false);
    ...
    const attempts = await db
      ...
      .from(resultAttempts)
      .where(and(eq(resultAttempts.simJobId, childJobId), eq(resultAttempts.aoaDeg, -5)));
  ...
  it("MUST-CATCH: a second crash through the same running-partial path escalates — row stays failed, needs_review counts it", async () => {
    const errSpy = vi.spyOn(console, "error");
    // The gap finder re-claimed the cell into a fresh precalc child.
    ...
    await db
      ...
      .where(and(eq(results.airfoilId, airfoilId), eq(results.simulationPresetRevisionId, revisionAId), eq(results.aoaDeg, -5)));

    await reconcile(db, runningPartialEngine(`${PREFIX}-engine-w2b`, SPEED_A, [divergedPrecalcPoint(-5)], 1), {
      jobIds: [child2JobId],
      skipFailedRecovery: true,
    });
    ...
    const buckets = await campaignReviewBuckets(db, campaignAId);
    expect(buckets.needsReview).toBe(1);
    expect(buckets.awaitingUrans).toBe(0);

    expect(errSpy.mock.calls.some((call) => String(call[0]).includes("AUTO-RETRY EXHAUSTED"))).toBe(true);
  }, 240000);
  ...
  it("MUST-CATCH: one crashed point inside an otherwise-COMPLETED multi-point campaign job requeues once (batched-partial shape)", async () => {
    const errSpy = vi.spyOn(console, "error");
    const composed = await composeCampaignJob(campaignBId, `${PREFIX}-engine-b1`);
    ...
    await reconcile(
      ...
      terminalEngine(`${PREFIX}-engine-b1`, "completed", SPEED_B, [
        solvedRansPoint(0),
        solvedRansPoint(5),
        { ...solvedRansPoint(-5), converged: false, cl: undefined, cd: undefined, cm: undefined, cl_cd: undefined, unsteady: false, error: DIVERGENCE_MESSAGE } as unknown as PolarPoint,
      ]),
    ...
    expect(points.find((p) => Number(p.aoaDeg) === -5)?.state).toBe("requested");
