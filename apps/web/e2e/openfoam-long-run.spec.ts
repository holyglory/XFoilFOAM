...
const detectedCpu = typeof os.availableParallelism === "function" ? os.availableParallelism() : os.cpus().length;
const effectiveCpu = Math.min(requestedCpu, Number(process.env.OPENFOAM_LOCAL_CPU_CAP ?? String(detectedCpu)));
const monitorIntervalMs = Number(process.env.OPENFOAM_MONITOR_INTERVAL_MS ?? "600000");
const smokeTimeoutMs = Number(process.env.OPENFOAM_SMOKE_TIMEOUT_MS ?? String(12 * 60 * 60 * 1000));
const allTimeoutMs = Number(process.env.OPENFOAM_ALL_TIMEOUT_MS ?? String(7 * 24 * 60 * 60 * 1000));
...
test.describe.serial("long OpenFOAM sweep verification", () => {
  test.skip(process.env.RUN_OPENFOAM_LONG !== "1", "Set RUN_OPENFOAM_LONG=1 to launch real OpenFOAM sweeps.");
  test.setTimeout(0);

  let artifactDir = "";

  test.beforeAll(async () => {
    artifactDir = path.resolve(process.cwd(), ".codex-artifacts", "openfoam-long-run", new Date().toISOString().replace(/[:.]/g, "-"));
  ...
  test("NACA0012 selected-profile smoke sweep", async ({ page, request }) => {
    ...
    await patchSweeper(request, { enabled: false, maxConcurrentJobs: effectiveCpu });
    ...
    if (pausedQueue.pendingSweepsTotal > 0) {
      ...
      const nacaPending = pausedQueue.pendingSweeps.find((row) => row.airfoilSlug === "naca-0012");
    ...
    const detail = await json<DetailPayload>(request, "get", "/api/airfoils/naca-0012");
    const validPoints = detail.polars.reduce((sum, polar) => sum + polar.points.filter((point) => point.resultId).length, 0);
    expect(validPoints, "NACA0012 Detail should expose stored valid solver points after the smoke run").toBeGreaterThan(0);

    await page.goto("/airfoils/naca-0012");
    await expect(page.getByRole("heading", { name: /NACA 0012/i })).toBeVisible();
  ...
  test("same sweep over all real profiles", async ({ page, request }) => {
    test.skip(process.env.RUN_OPENFOAM_ALL !== "1", "Set RUN_OPENFOAM_ALL=1 after the NACA smoke passes to run every profile.");
    ...
    await monitorQueue(page, request, {
      ...
      timeoutMs: allTimeoutMs,
        ...
        return queue.pendingSweepsTotal === 0 && queue.activeJobs.length === 0 && engineQueued === 0 && engineActive === 0 && engineReserved === 0 && stale === 0;
      },
    });
  ...
  async function ensureLongRunSetup(request: APIRequestContext) {
    ...
    expect(air?.id, "Air medium must be seeded before launching OpenFOAM sweeps").toBeTruthy();
    const naca0012 = initialSetup.airfoilOptions.find((airfoil) => airfoil.slug === "naca-0012");
    expect(naca0012?.id, "NACA 0012 must be seeded from real geometry").toBeTruthy();

    const flow = await ensureRecord<SetupRecord>(request, "flowConditions", "/api/admin/flow-conditions", {
    ...
    const reference = await ensureRecord<SetupRecord>(request, "referenceGeometryProfiles", "/api/admin/reference-geometry-profiles", {
    ...
    const boundary = await ensureRecord<SetupRecord>(request, "boundaryProfiles", "/api/admin/boundary-profiles", {
    ...
    const mesh = await ensureRecord<SetupRecord>(request, "meshProfiles", "/api/admin/mesh-profiles", {
    ...
    const solver = await ensureRecord<SetupRecord>(request, "solverProfiles", "/api/admin/solver-profiles", {
    ...
    const scheduling = await ensureRecord<SetupRecord>(request, "schedulingProfiles", "/api/admin/scheduling-profiles", {
    ...
    const output = await ensureRecord<SetupRecord>(request, "outputProfiles", "/api/admin/output-profiles", {
      slug: `${LONG_RUN_SLUG}-output`,
      name: `${LONG_RUN_NAME} output`,
      writeImages: ["velocity_magnitude", "pressure"],
      imageZoomChords: 2,
    });
    const sweep = await ensureRecord<SetupRecord>(request, "sweepDefinitions", "/api/admin/sweep-definitions", {
    ...
    const existing = setup.simulationPresets.find((row) => row.slug === LONG_RUN_SLUG);
    const preset = existing
      ? await patchPreset(request, existing.id, withoutSlug(presetBody))
      : await json<SimulationPresetRow>(request, "post", "/api/admin/simulation-presets", presetBody);

    await fs.writeFile(
      path.join(artifactDir, "setup.json"),
      JSON.stringify({ requestedCpu, detectedCpu, effectiveCpu, preset, naca0012, createdAt: new Date().toISOString() }, null, 2),
    );
  ...
  async function ensureRecord<T extends SetupRecord>(
    ...
    collection: keyof Pick<
      SimulationSetup,
      "flowConditions" | "referenceGeometryProfiles" | "boundaryProfiles" | "meshProfiles" | "solverProfiles" | "schedulingProfiles" | "outputProfiles" | "sweepDefinitions"
    >,
    endpoint: string,
    body: Record<string, unknown> & { slug: string; name: string },
  ): Promise<T> {
    const setup = await getSetup(request);
    const existing = (setup[collection] as SetupRecord[]).find((row) => row.slug === body.slug);
    if (existing) {
      return json<T>(request, "patch", `${endpoint}/${existing.id}`, withoutSlug(body));
    }
    return json<T>(request, "post", endpoint, body);
  }

  async function disableOtherPresets(request: APIRequestContext, keepPresetId: string) {
    const setup = await getSetup(request);
    await Promise.all(
  ...
  async function verifyPresetScopeInUi(page: Page, presetName: string, expectedScope: "all" | "airfoils", expectedSelection?: string) {
    await page.goto("/admin");
    await page.getByRole("button", { name: /Simulation setup/i }).click();
    await page.getByRole("button", { name: /^Presets$/i }).click();
    await page.getByRole("button", { name: new RegExp(escapeRegExp(presetName)) }).click();
    await expect(wrappedSelect(page, "Run scope")).toHaveValue(expectedScope);
    if (expectedScope === "airfoils") {
      await expect(page.getByTestId("preset-airfoil-selected-count")).toContainText(expectedSelection ?? "1 selected");
      await page.getByTestId("preset-airfoil-search").fill("NACA 0012");
      await expect(page.getByTestId("preset-airfoil-option-naca-0012")).toBeVisible();
    }
  }

  async function openQueue(page: Page) {
    await page.goto("/admin");
    await page.getByRole("button", { name: /OpenFOAM queue/i }).click();
  ...
  async function monitorQueue(
    page: Page,
    request: APIRequestContext,
    opts: { label: string; timeoutMs: number; done: (queue: AdminQueue) => boolean },
  ) {
    ...
    for (let iteration = 0; ; iteration++) {
      ...
      await capture(page, `${opts.label}-${String(iteration).padStart(4, "0")}`);
      ...
      if (staleCount > 0) {
        if (recoveredStale) {
          ...
          throw new Error(`Stale jobs recurred after recovery during ${opts.label}.`);
        }
        recoveredStale = true;
        const recover = page.getByRole("button", { name: /recover stale/i });
        if ((await recover.count()) > 0) await recover.first().click();
        else await json(request, "post", "/api/admin/jobs/recover-stale", { olderThanMinutes: 30 });
      }

      if (Date.now() - startedAt > opts.timeoutMs) {
        await patchSweeper(request, { enabled: false });
        throw new Error(`${opts.label} did not finish within ${opts.timeoutMs}ms.`);
      }
  ...
  async function preflight(request: APIRequestContext) {
    ...
    const queue = await getQueue(request);
    expect(queue.engineQueueError, "OpenFOAM engine queue should be visible before launching a long run").toBeNull();
  }

  async function assertNoSyntheticAirfoils(request: APIRequestContext) {
    const catalog = await json<{ items: Array<{ slug: string; name: string }> }>(request, "get", "/api/airfoils?limit=10000");
    const synthetic = catalog.items.filter((airfoil) => airfoil.slug.startsWith("pw-") || airfoil.name.startsWith("pw-"));
    expect(synthetic, "Long OpenFOAM runs must not include Playwright synthetic airfoils").toEqual([]);
  }

  async function getSetup(request: APIRequestContext): Promise<SimulationSetup> {
  ...
  async function patchSweeper(request: APIRequestContext, body: Record<string, unknown>) {
    return json(request, "patch", "/api/admin/sweeper", body);
  }

  async function patchPreset(request: APIRequestContext, id: string, body: Record<string, unknown>): Promise<SimulationPresetRow> {
    return json<SimulationPresetRow>(request, "patch", `/api/admin/simulation-presets/${id}`, body);
  }
...
async function json<T>(request: APIRequestContext, method: "get" | "post" | "patch", urlPath: string, data?: unknown): Promise<T> {
  const res = await request[method](`${apiURL}${urlPath}`, data === undefined ? undefined : { data });
  if (!res.ok()) {
    expect(false, `${method.toUpperCase()} ${urlPath} -> ${res.status()} ${await res.text().catch(() => "")}`).toBeTruthy();
  }
