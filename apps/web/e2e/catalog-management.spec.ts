...
async function createCategory(
  ...
  return json<CategoryRow>(request, "post", "/api/admin/categories", {
    name,
    parentId: parentId ?? null,
  });
}

async function createAirfoil(
  request: APIRequestContext,
  name: string,
  categorySlug: string,
  t: number,
  ...
  return json<AirfoilRow>(request, "post", "/api/airfoils", {
    name,
    categorySlug,
    naca: { t, m, p },
  });
...
async function listMediums(request: APIRequestContext): Promise<MediumRow[]> {
  return (
    await json<{ items: MediumRow[] }>(request, "get", "/api/admin/mediums")
  ).items;
}

async function listBoundaryConditions(
  request: APIRequestContext,
): Promise<SimulationPresetCompatRow[]> {
  return (
    await json<{ items: SimulationPresetCompatRow[] }>(
      request,
      "get",
...
async function getSimulationSetup(
  request: APIRequestContext,
): Promise<SimulationSetupRow> {
  return json<SimulationSetupRow>(
    request,
    "get",
...
async function dragCategory(
  ...
  await source.dragTo(target, {
    targetPosition: {
      x: Math.max(12, box!.width / 2),
      y: Math.max(2, Math.min(box!.height - 2, box!.height * yRatio)),
    },
  });
}
...
function isAirfoilQueryFor(
  ...
  try {
    ...
    return (
      ...
      url.searchParams.get("includeSubcategories") ===
...
async function selectBrowseCategory(
  page: Page,
  categorySlug: string,
  includeSubcategories = true,
) {
  await expect(page.getByTestId("browse-surface")).toHaveAttribute(
    "data-hydrated",
    "true",
  );
  await Promise.all([
    page.waitForResponse(
      (res) =>
        res.ok() &&
        isAirfoilQueryFor(res.url(), categorySlug, includeSubcategories),
    ),
    page.getByTestId(`category-${categorySlug}`).click(),
  ]);
}

async function setBrowseIncludeSubcategories(
  page: Page,
  checked: boolean,
  categorySlug: string,
) {
  ...
  await Promise.all([
    page.waitForResponse(
      (res) => res.ok() && isAirfoilQueryFor(res.url(), categorySlug, checked),
    ),
    checked
      ? page.getByTestId("include-subcategories").check()
      : page.getByTestId("include-subcategories").uncheck(),
  ]);
}

test.describe
  .serial("catalog tree, filters, hashtags, and bulk management", () => {
  test.beforeAll(async ({ request }) => {
    state.stamp = `pw-${Date.now().toString(36)}`;
    state.root = await createCategory(request, `${state.stamp} Root`);
    state.child = await createCategory(
      request,
      `${state.stamp} Child`,
      state.root.id,
    );
    ...
    state.rootAirfoil = await createAirfoil(
      request,
      `${state.stamp} Root 0012`,
      state.root.slug,
      0.12,
      0,
      0,
    );
    state.childAirfoil = await createAirfoil(
      request,
      `${state.stamp} Child 4415`,
      state.child.slug,
      0.15,
      0.04,
      0.4,
    );
    state.removeAirfoil = await createAirfoil(
      request,
      `${state.stamp} Remove 0009`,
      state.root.slug,
      0.09,
      0,
      0,
  ...
  test("admin preset save validates missing preset name without creating a row", async ({
    page,
    request,
  }) => {
    ...
    const saveButton = page.getByRole("button", {
      name: /save new simulation preset/i,
    });
    await expect(saveButton).toBeEnabled();
    await saveButton.click();

    await expect(
      page.getByText("Preset name is required").first(),
    ).toBeVisible();
    await expect(page.getByLabel("Preset name", { exact: true })).toBeFocused();
    expect((await getSimulationSetup(request)).simulationPresets).toHaveLength(
      before,
    );
  });

  test("admin setup create and update actions validate required fields instead of disabling", async ({
    page,
  }) => {
    await page.goto("/admin");
    await gotoAdminSection(page, "setup");
    ...
    const cases = [
      {
        tab: "Flow state",
        button: /add flow state/i,
        message: "Name is required",
      },
      {
        tab: "Reference geometry",
        button: /add reference geometry/i,
        message: "Name is required",
      },
      {
        tab: "Boundary",
        button: /add boundary profile/i,
        message: "Name is required",
      },
      { tab: "Mesh", button: /add mesh profile/i, message: "Name is required" },
      {
        tab: "Solver",
        button: /add solver profile/i,
        message: "Name is required",
      },
      {
        tab: "Scheduling",
        button: /add scheduling profile/i,
        message: "Name is required",
      },
      {
        tab: "Output",
        button: /add output profile/i,
        message: "Name is required",
      },
      {
        tab: "Sweeps",
        button: /add sweep definition/i,
        message: "Name is required",
      },
      {
        tab: "Presets",
        button: /save new simulation preset/i,
    ...
    for (const item of cases) {
      await page
        .getByRole("button", { name: new RegExp(`^${item.tab}$`, "i") })
        .click();
  ...
  test("adjacent admin forms validate missing user input without silent disabled buttons", async ({
    page,
  }) => {
    await page.goto("/admin");
    ...
    await expect(
      page.getByRole("button", { name: /add medium/i }),
    ).toBeEnabled();
    ...
    await expect(
      page.getByRole("button", { name: /add hashtag/i }),
    ).toBeEnabled();
    ...
    await expect(
      page.getByRole("button", { name: /^Add airfoil$/i }),
    ).toBeEnabled();
    ...
    await expect(
      page.getByRole("button", { name: /sync DB \+ remote refs/i }),
    ).toBeEnabled();
    await page.getByRole("button", { name: /sync DB \+ remote refs/i }).click();
    await expect(
      page.getByText("Up-tier endpoint is required").first(),
    ).toBeVisible();
  });

  test("browse tree scopes direct and descendant airfoils", async ({
    page,
  }) => {
    await page.goto("/");
    await selectBrowseCategory(page, state.root.slug);
    await expect(
      page.getByTestId(`airfoil-row-${state.rootAirfoil.slug}`),
    ).toBeVisible();
    await expect(
      page.getByTestId(`airfoil-row-${state.childAirfoil.slug}`),
    ).toBeVisible();
  ...
  test("browse row chooses the Detail profile while checkbox stays selection-only", async ({
    page,
  }) => {
    await page.goto("/");
    await selectBrowseCategory(page, state.root.slug);
    ...
    await expect(page).toHaveURL(
      new RegExp(`/airfoils/${state.childAirfoil.slug}$`),
    );
    await expect(
      page.getByRole("heading", { name: state.childAirfoil.name }),
    ).toBeVisible();
  ...
  test("advanced filters apply thickness, area UI, and hashtag filters", async ({
    page,
  }) => {
    await page.goto("/");
    await selectBrowseCategory(page, state.root.slug);
  ...
  test("selection controls appear on hover and select all/none works", async ({
    page,
  }) => {
    await page.goto("/");
    await selectBrowseCategory(page, state.root.slug);
  ...
  test("bulk move persists category assignment", async ({ page }) => {
    ...
    await page
      .getByTestId("bulk-move-popover")
      .getByRole("button", { name: new RegExp(state.dest.name) })
      .click();
    await expect(
      page.getByTestId(`airfoil-row-${state.childAirfoil.slug}`),
    ).toHaveCount(0);
  ...
  test("bulk hashtag assign and remove changes filter results", async ({
    page,
  }) => {
    await page.goto("/");
    await selectBrowseCategory(page, state.dest.slug);
    ...
    await page
      .getByTestId("bulk-hashtag-popover")
      .getByRole("button", { name: new RegExp(state.tag2.name) })
      .click();
    ...
    await page
      ...
      .click();
    await page
      .getByRole("button", { name: /remove selected hashtags/i })
      .click();
    await expect(
      page.getByTestId(`airfoil-row-${state.childAirfoil.slug}`),
    ).toHaveCount(0);
  ...
  test("bulk archive and remove hide selected airfoils", async ({ page }) => {
    ...
    page.once("dialog", (dialog) => dialog.accept());
    await page
      .getByRole("button", { name: /remove selected airfoils/i })
      .click();
    await expect(
      page.getByTestId(`airfoil-row-${state.removeAirfoil.slug}`),
    ).toHaveCount(0);
  });

  test("admin category editor creates, renames, blocks nonempty delete, moves, and deletes empty category", async ({
    page,
  }) => {
    await page.goto("/admin");
    await openCatalogTab(page, "categories");
    ...
    await page.getByRole("button", { name: /create category/i }).click();
    const createdButton = page.getByRole("button", {
      name: new RegExp(`${state.stamp} Empty`),
    });
    await expect(createdButton).toBeVisible();

    await createdButton.click();
    await page
      .getByTestId("edit-category-name")
      .fill(`${state.stamp} Empty Renamed`);
    await page.getByTestId("edit-category-parent").selectOption(state.dest.id);
    await page.getByRole("button", { name: /^save$/i }).click();
    await expect(
      page.getByRole("button", {
        name: new RegExp(`${state.stamp} Empty Renamed`),
      }),
    ).toBeVisible();
    ...
    await page
      .getByRole("button", { name: new RegExp(`${state.stamp} Empty Renamed`) })
      .click();
    page.once("dialog", (dialog) => dialog.accept());
    await page.getByRole("button", { name: /^delete$/i }).click();
    await expect(
  ...
  test("admin category tree supports drag reparent and reorder", async ({
    page,
    request,
  }) => {
    ...
    const aBox = await page
      .getByTestId(`admin-category-${dragA.slug}`)
      .boundingBox();
    const bBox = await page
      .getByTestId(`admin-category-${dragB.slug}`)
      .boundingBox();
  ...
  test("admin Solver page shows activity, background, and engine surfaces", async ({
    page,
    request,
  }) => {
    await page.goto("/admin");
    ...
    await expect(page.getByText("Solver CPU slots")).toBeVisible();
    await expect(page.getByTestId("queue-active-jobs")).toContainText(
      "Active jobs",
    );
    await expect(page.getByTestId("queue-finished-jobs")).toContainText(
      "Finished job log",
    );
    ...
    const solvedPoints = await json<{ items: unknown[]; solvedToday: number }>(
      request,
      "get",
      "/api/admin/solved-points?limit=1",
    );
    const solvedBadge = page.getByTestId("solved-today-badge");
    if (solvedPoints.solvedToday > 0) {
      await expect(solvedBadge).toContainText(
        `${solvedPoints.solvedToday.toLocaleString()} solved today`,
      );
      ...
      await expect(
        popover.getByTestId("solved-point-row").first(),
      ).toBeVisible();
    ...
    await expect(page.getByTestId("queue-pending-sweeps")).toContainText(
      "Pending sweeps",
    );
    await expect(
      page.getByText(/AoA sweep|No pending sweeps/).first(),
    ).toBeVisible();
    await expect(page.getByTestId("queue-external-promises")).toContainText(
      "Externally promised",
    );
    ...
    await expect(page.getByTestId("engine-cache-card")).toContainText(
      /Mesh entries|cache stats unavailable/,
    );
    await expect(page.getByTestId("engine-recover-stale")).toContainText(
      "recover stale",
    );
  ...
  test("admin creates and edits mediums from the UI", async ({
    page,
    request,
  }) => {
    ...
    const medium = (await listMediums(request)).find(
      (m) => m.slug === slugify(name),
    );
    expect(medium?.name).toBe(name);
  });

  test("admin creates and edits simulation setup presets with derived Re and Mach", async ({
    page,
    request,
  }) => {
    ...
    await fillWrappedField(
      page,
      "Slug optional",
      `${slugify(bcName)}-reference-geometry`,
    );
    ...
    await expect(
      page.getByLabel("Turbulent viscosity ratio νt/ν"),
    ).toBeVisible();
    const viscosityPresetBox = await page
      .getByLabel("Turbulent viscosity ratio νt/ν")
      .boundingBox();
    ...
    await expect(
      meshGuide.getByRole("img", { name: /c-grid airfoil mesh infographic/i }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Chordwise wall cells along the airfoil surface"),
    ).toBeVisible();
    await expect(
      page.getByText("The mesh stays chord-aligned while AoA sweeps rotate"),
    ).toBeVisible();
    ...
    expect(explanationBox?.y ?? 0).toBeGreaterThan(
      (artifactBox?.y ?? 0) + (artifactBox?.height ?? 0) - 1,
    );
    ...
    const originalMeshRow = page.getByRole("button", {
      name: new RegExp(`${bcName} mesh blockmesh`),
    });
    const variantMeshRow = page.getByRole("button", {
      name: new RegExp(`${bcName} mesh variant blockmesh`),
    });
    ...
    await page
      .getByRole("button", { name: /save as new mesh profile/i })
      .click();
    await expect(variantMeshRow).toBeVisible();
    await expect(originalMeshRow).toBeVisible();
    await expect(
      page.getByRole("button", { name: /remove selected/i }),
    ).toHaveCount(0);
    await page
      .getByRole("button", {
        name: new RegExp(`^Remove ${escapeRegExp(`${bcName} mesh variant`)}$`),
      })
      .click();
    ...
    await expect(
      page.getByText("Draft changes are not saved automatically").first(),
    ).toBeVisible();
    ...
    await selectWrappedField(
      page,
      "Reference geometry",
      `${bcName} reference geometry`,
    );
    ...
    await selectWrappedField(page, "Enabled", "no");
    await page
      .getByRole("button", { name: /save new simulation preset/i })
      .click();

    await expect(
      page.getByRole("button", { name: new RegExp(bcName) }),
    ).toBeVisible();

    await page.getByRole("button", { name: /^Flow state$/i }).click();
    await page
      .getByRole("button", {
        name: new RegExp(`^(?!Remove )${escapeRegExp(`${bcName} flow`)}`),
      })
      .click();
    ...
    await page
      .getByRole("button", { name: /update selected flow state/i })
      .click();

    await expect
      .poll(
        async () =>
          (await listBoundaryConditions(request)).find(
            (item) => item.slug === slugify(bcName),
          )?.speedMps,
        {
      ...
      .toBe(50);
    const saved = (await listBoundaryConditions(request)).find(
      (item) => item.slug === slugify(bcName),
    );
  ...
  test("admin hashtag editor creates, renames, and deletes hashtags", async ({
    page,
  }) => {
    ...
    await page.getByRole("button", { name: /add hashtag/i }).click();
    await expect(
      page.getByTestId(`hashtag-name-${slugify(name)}`),
    ).toBeVisible();
    ...
    await page
      ...
      .click();
    await expect(
      page.getByTestId(`hashtag-name-${slugify(renamed)}`),
    ).toBeVisible();
