...
describe("verify filter URL round-trip", () => {
  it("round-trips pending and disagreed and preserves foreign params", () => {
    for (const verify of ["pending", "disagreed"] as const) {
      const search = pointFiltersToSearch("?section=queue&tab=points", {
        ...DEFAULT_POINT_FILTERS,
        verify,
      });
  ...
  it("omits the default ('' = any) and rejects malformed values", () => {
    ...
    const cleared = pointFiltersToSearch(
      "?pverify=pending",
      DEFAULT_POINT_FILTERS,
    );
...
describe("disagreedDeltaLabel", () => {
  it("lists only the deltas that exceeded their contract bound", () => {
    ...
    expect(
      disagreedDeltaLabel(
        verifyInfo({ state: "disagreed", deltaCl: 0.01, deltaCd: -0.012 }),
      ),
    ).toBe("Δcd 0.012");
    expect(
      disagreedDeltaLabel(
        verifyInfo({ state: "disagreed", deltaCl: -0.07, deltaCd: 0.02 }),
      ),
    ).toBe("Δcl 0.07 · Δcd 0.02");
  });

  it("falls back to every recorded delta when none exceeds a bound (defensive honesty)", () => {
    expect(
      disagreedDeltaLabel(
        verifyInfo({ state: "disagreed", deltaCl: 0.01, deltaCd: 0.001 }),
      ),
    ).toBe("Δcl 0.01 · Δcd 0.001");
...
describe("assembleTimeline — verify events", () => {
  ...
  it("no verify item / settled clean → no verify event", () => {
    expect(
      assembleTimeline(storyWith(null)).some((e) =>
        e.title.includes("verification"),
      ),
    ).toBe(false);
    expect(
      assembleTimeline(storyWith(verifyInfo({ state: "done" }))).some((e) =>
        e.title.includes("verification"),
      ),
    ).toBe(false);
  });
...
describe("buildSteadyHistoryModel", () => {
  ...
  it("truncates to the shortest series when lengths drift (never pads)", () => {
    const m = buildSteadyHistoryModel({
      ...steadyHistory(),
      cm: [-0.1, -0.11, -0.1],
    })!;
...
describe("campaignPhaseBadge", () => {
  ...
  it("maps the three running phases to their labels", () => {
    expect(campaignPhaseBadge("running_rans", "active", null)).toEqual({
      key: "running_rans",
      label: "RUNNING RANS",
      tone: "teal",
    });
    expect(campaignPhaseBadge("running_precalc", "active", null)).toEqual({
      key: "running_precalc",
      label: "RUNNING PRECALC URANS",
      tone: "amber",
    });
    expect(campaignPhaseBadge("running_refinement", "active", null)).toEqual({
      key: "running_refinement",
      label: "RUNNING URANS REFINEMENT",
      tone: "amber",
    });
  });
  ...
  it("no phase / no payload → no badge; completed lifecycle suppresses the duplicate", () => {
    ...
    expect(campaignPhaseBadge("completed", "active", null)).toEqual({
      key: "completed",
      label: "ALL TIERS TERMINAL",
      tone: "teal",
    });
  });
});
...
describe("tierCountsLine", () => {
  it("renders the three real per-tier open counts", () => {
    expect(
      tierCountsLine({ ransOpen: 1200, precalcOpen: 3, verifyOpen: 0 }),
    ).toBe("tiers open — RANS 1,200 · precalc 3 · verify 0");
  });
