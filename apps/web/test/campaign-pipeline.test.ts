...
const totals = (
  over: Partial<Parameters<typeof progressBarSegments>[0]> = {},
) => ({
  requested: 1750,
  solved: 1200,
  failed: 0,
...
describe("progressBarSegments", () => {
  it("teal done = solved + derived; amber = running; violet = awaitingUrans; rest open", () => {
    const seg = progressBarSegments(
      totals({ running: 12, rejected: 96, remaining: 402 }),
  ...
  it("failed / needs-review are NOT bar segments (they live on the red chip)", () => {
    const seg = progressBarSegments(totals({ failed: 5 }), {
      awaitingUrans: 0,
      needsReview: 5,
    });
  ...
  it("requested = 0 never divides by zero; open never goes negative", () => {
    ...
    const overfull = progressBarSegments(totals({ solved: 2000 }), {
      awaitingUrans: 100,
      needsReview: 0,
    });
...
describe("stageEta", () => {
  ...
  it("MUST-HIDE: no rate payload, zero measured rate, or missing tier counts", () => {
    ...
    expect(
      stageEta({
        phase: "running_rans",
        stageOpenByPhase: opens,
        rate: { pointsLast24h: 0, measuredSince: iso(6 * 3600_000) },
        nowMs: NOW,
      }),
    ).toBeNull();
    expect(
  ...
  it("MUST-HIDE: unstable measurement window (< 1 h of history) — startup noise is not a rate", () => {
    const fresh = {
      pointsLast24h: 96,
      measuredSince: iso(ETA_MIN_WINDOW_MS - 60_000),
    };
    expect(
      stageEta({
        phase: "running_rans",
    ...
    const exactly = {
      pointsLast24h: 96,
      measuredSince: iso(ETA_MIN_WINDOW_MS),
    };
    expect(
      stageEta({
        phase: "running_rans",
  ...
  it("MUST-HIDE: no running phase (completed/paused) or an empty current stage", () => {
    ...
    expect(
      stageEta({
        phase: "running_rans",
        stageOpenByPhase: { ...opens, ransOpen: 0 },
        rate: stableRate,
        nowMs: NOW,
      }),
    ).toBeNull();
  ...
  it("malformed measuredSince hides instead of NaN math", () => {
    expect(
      stageEta({
        phase: "running_rans",
        stageOpenByPhase: opens,
        rate: { pointsLast24h: 96, measuredSince: "not-a-date" },
        nowMs: NOW,
      }),
    ).toBeNull();
  });
...
describe("sweepChipLabel", () => {
  it("range sweeps render from/to/step; list sweeps render the angle count", () => {
    ...
    ).toBe("sweep -4°…14° step 2°");
    expect(
      sweepChipLabel({
        fromDeg: null,
        toDeg: null,
        stepDeg: null,
        listDeg: ["0.0000", "4.0000", "8.0000"],
      }),
    ).toBe("sweep 3 angles");
    expect(
      sweepChipLabel({
