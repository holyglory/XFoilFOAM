...
import { type AirfoilSummary, type PolarMetricCondition } from "@aerodb/core";
...
import {
  ...
  rankSearchItems,
  SEARCH_OBJECTIVES,
} from "@/components/search/ranking";
...
export function SearchView({
  ...
  const ranked = useMemo(() => {
    return rankSearchItems(items, objKey, {
      clmaxOn,
      clmaxMin,
      cdminOn,
      cdminMax,
    ...
    <div style={{ maxWidth: 980, margin: "0 auto" }}>
      <div style={{ margin: "12px 0 18px" }}>
        <MetricConditionSelector
          ...
          value={conditionKey}
          onChange={(key) =>
            router.replace(withMetricCondition("/search", key))
          }
        />
      </div>
      <div style={card}>
        ...
        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
          <Constraint
            on={clmaxOn}
            toggle={() => setClmaxOn((v) => !v)}
            text={`Cl,max ≥ ${clmaxMin.toFixed(2)}`}
          >
            <input
              type="range"
              min={0.5}
              ...
              step={0.05}
              value={clmaxMin}
              onChange={(e) => setClmaxMin(+e.target.value)}
              disabled={!clmaxOn}
            />
          </Constraint>
          <Constraint
            on={cdminOn}
            toggle={() => setCdminOn((v) => !v)}
            text={`Cd,min ≤ ${cdminMax.toFixed(4)}`}
          >
            <input
              type="range"
              min={0.004}
              ...
              step={0.0005}
              value={cdminMax}
              onChange={(e) => setCdminMax(+e.target.value)}
              disabled={!cdminOn}
            />
          </Constraint>
          <Constraint
      ...
      <div style={{ ...card, padding: 0 }}>
        <div style={{ ...label, padding: "14px 16px 0" }}>
          RANKED · {ranked.length} match{ranked.length === 1 ? "" : "es"}
        </div>
        {ranked.length === 0 ? (
          <div
            style={{
          ...
          >
            No calculated polar points satisfy these constraints yet.
          </div>
        ) : (
          ranked.map(({ airfoil: a, value }, i) => (
            <Link
              ...
              href={withMetricCondition(
                `/airfoils/${a.slug}`,
                conditionKey || a.metricCondition?.key,
              )}
              style={{
                display: "flex",
                alignItems: "center",
