...
// Points tab per point), the red "needs review · N" chip (only when N > 0)
...
export function CampaignDetail({
  ...
  }, [planEditMode, addAirfoilsOpen, cancelConfirm, forceReleaseTarget, onBack]);
  ...
  const lineColor =
    line.tone === "teal" ? C.teal : line.tone === "amber" ? C.amber : line.tone === "red" ? C.redText : line.tone === "violet" ? C.violet : C.dim;
  ...
  const eta = stageEta({
    ...
    stageOpenByPhase: summary.tierCounts
      ? {
          ...
          unsteadyOpen: summary.tierCounts.precalcOpen + (reviewBuckets?.awaitingUrans ?? 0),
          verifyOpen: summary.tierCounts.verifyOpen,
        }
      : null,
    rate: rate ? { pointsLast24h: rate.pointsLast24h, measuredSince: rate.measuredSince } : null,
  });
  ...
  const primaryAction: ActionDef | null =
    status === "active" || status === "attention"
      ? { key: "pause", label: busyAction === "pause" ? "pausing…" : "Pause", onClick: () => void runVerb("pause"), testId: "campaign-action-pause" }
      : status === "paused"
        ? { key: "resume", label: busyAction === "resume" ? "resuming…" : "Resume", tone: "primary", onClick: () => void runVerb("resume"), testId: "campaign-action-resume" }
        : null;
  ...
  menuActions.push({ key: "duplicate", label: busyAction === "duplicate" ? "preparing…" : "Duplicate", onClick: () => void duplicate(), testId: "campaign-action-duplicate" });
  if (status === "attention") {
    menuActions.push({
  ...
  if (["active", "paused", "attention"].includes(status)) {
    menuActions.push({ key: "cancel", label: "Cancel…", tone: "red", onClick: () => setCancelConfirm(true), testId: "campaign-action-cancel" });
  }
  if (["completed", "cancelled"].includes(status)) {
    menuActions.push({ key: "archive", label: busyAction === "archive" ? "archiving…" : "Archive", onClick: () => void runVerb("archive"), testId: "campaign-action-archive" });
  }
  if (status === "archived") {
    menuActions.push({ key: "unarchive", label: busyAction === "archive" ? "restoring…" : "Unarchive", onClick: () => void runVerb("archive"), testId: "campaign-action-unarchive" });
  }
  ...
  return (
    <div data-testid="campaign-detail" style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        {/* Red "needs review · N" chip (approved design D): renders ONLY when
            N > 0, never for the calm violet awaiting-URANS queue. Click opens
            Solver ▸ Points filtered to needs_review — the repair verbs
            (Retry / Continue) live THERE per point, not in this header. */}
        {needsReview > 0 && (
          <button
            type="button"
          ...
          >
            needs review · {fCount(needsReview)}
          </button>
        )}
      </div>
      ...
      {pipeline && (
        <div
          ...
          style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", display: "flex", gap: 8, alignItems: "stretch", flexWrap: "wrap" }}
        >
          {pipeline.stages.map((stage, i) => (
            <div key={stage.key} style={{ display: "flex", alignItems: "stretch", gap: 8, flex: "1 1 180px", minWidth: 0 }}>
              {i > 0 && <span aria-hidden style={{ alignSelf: "center", fontFamily: MONO, fontSize: 12, color: C.dimmer }}>→</span>}
              <div
              ...
              >
                <span style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: "0.08em", color: stage.active ? C.teal : C.dim }}>
                  {stage.title.toUpperCase()}
                </span>
                <span style={{ fontFamily: MONO, fontSize: 12.5, fontWeight: 700, color: stage.settled || stage.open === 0 ? C.dim : C.text }}>
                  {/* "—" = not started (upstream stages still feed this one);
                      "settled ✓" only when nothing can arrive any more. */}
                  {stage.settled ? "settled ✓" : stage.open === 0 ? "—" : stage.detail ?? `${fCount(stage.open)} open`}
                </span>
                {stage.active && pipeline.jobsRunning > 0 && (
                  <span style={{ fontFamily: MONO, fontSize: 9.5, color: C.amber }}>
                    {fCount(pipeline.jobsRunning)} job{pipeline.jobsRunning === 1 ? "" : "s"} solving now
                  </span>
                )}
                {PIPELINE_STAGE_NOTES[stage.key] && (
                  <span style={{ fontFamily: MONO, fontSize: 9, color: C.dimmest }}>{PIPELINE_STAGE_NOTES[stage.key]}</span>
                )}
              </div>
            </div>
          ))}
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", display: "grid", gap: 8 }}>
        <div data-testid="campaign-progress-bar" style={{ height: 8, background: C.panel3, borderRadius: 5, overflow: "hidden", display: "flex" }} aria-hidden>
          <span style={{ width: `${barSegments.done * 100}%`, background: C.teal, display: "block" }} />
          <span style={{ width: `${barSegments.solving * 100}%`, background: C.amber, display: "block" }} />
          <span style={{ width: `${barSegments.awaitingUrans * 100}%`, background: C.violet, display: "block" }} />
          {/* remainder of the track = open work (panel background) */}
        </div>
        <div data-testid="campaign-counts-line" style={{ fontFamily: MONO, fontSize: 10.5, color: C.muted, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ color: C.teal }}>
            <span aria-hidden style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: C.teal, marginRight: 4, verticalAlign: -1 }} />
            {fCount(barSegments.doneCount)} done
          </span>
          {barSegments.solvingCount > 0 && (
            <span style={{ color: C.amber }}>
              <span aria-hidden style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: C.amber, marginRight: 4, verticalAlign: -1 }} />
          ...
          {barSegments.awaitingCount > 0 && (
            <button
              ...
              style={{ fontFamily: MONO, fontSize: 10.5, color: C.violet, background: "transparent", border: "none", padding: 0, cursor: "pointer", textDecoration: "underline" }}
            >
              <span aria-hidden style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: C.violet, marginRight: 4, verticalAlign: -1 }} />
              {fCount(barSegments.awaitingCount)} awaiting URANS
            </button>
          )}
          <span>
            <span aria-hidden style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: C.panel3, border: `1px solid ${C.stroke}`, marginRight: 4, verticalAlign: -1 }} />
            {fCount(barSegments.openCount)} open of {fCount(totals.requested)}
          </span>
          {eta && (
            <span data-testid="campaign-eta" style={{ color: C.dim }} title="From the measured trailing-24h ingest rate — hidden while the rate is unstable">
          ...
          <button
            ...
            style={{ fontFamily: MONO, fontSize: 10.5, color: C.teal, background: "transparent", border: "none", padding: 0, cursor: "pointer" }}
          >
            {detailsOpen ? "▾ details" : "▸ details"}
          </button>
        </div>
        {detailsOpen && (
          <div data-testid="campaign-details" style={{ display: "grid", gap: 7, borderTop: `1px solid ${C.borderRow}`, paddingTop: 8 }}>
            <div style={{ fontFamily: MONO, fontSize: 10.5, color: C.muted, display: "flex", gap: 14, flexWrap: "wrap" }}>
            ...
            {tiersLine && (
              <div data-testid="campaign-tier-counts" style={{ fontFamily: MONO, fontSize: 10, color: C.dim }} title="Fidelity ladder: RANS gaps, then precalc URANS, then full-fidelity verification">
                {tiersLine}
              </div>
            )}
            {rate && (
            ...
            <div data-testid="campaign-plan-chips" style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <span style={{ fontFamily: MONO, fontSize: 10, color: C.muted, border: `1px solid ${C.stroke}`, borderRadius: 999, padding: "3px 9px" }}>polar sweep</span>
              {sweepChip && (
                <span style={{ fontFamily: MONO, fontSize: 10, color: C.muted, border: `1px solid ${C.stroke}`, borderRadius: 999, padding: "3px 9px" }}>{sweepChip}</span>
              )}
