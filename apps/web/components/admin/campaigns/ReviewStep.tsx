...
const PRIORITY_OPTIONS = [
  { value: "0", label: "Background (0) — interleaves with continuous gap-fill in Re/airfoil/angle order" },
  { value: "5", label: "Standard (5) — runs before continuous gap-fill" },
  { value: "8", label: "High (8) — runs before Standard campaigns and can starve them" },
];

type NumericsSlot = "boundaryProfileId" | "meshProfileId" | "solverProfileId" | "outputProfileId";
...
const NUMERICS_SLOTS: Array<{
  ...
  rows: keyof Pick<AdminSimulationSetup, "boundaryProfiles" | "meshProfiles" | "solverProfiles" | "outputProfiles">;
}> = [
  { slot: "boundaryProfileId", label: "Boundary", field: "Boundary profile", kind: "boundary", rows: "boundaryProfiles" },
  { slot: "meshProfileId", label: "Mesh", field: "Mesh profile", kind: "mesh", rows: "meshProfiles" },
  { slot: "solverProfileId", label: "Solver", field: "Solver profile", kind: "solver", rows: "solverProfiles" },
  { slot: "outputProfileId", label: "Output", field: "Output profile", kind: "output", rows: "outputProfiles" },
];
...
export interface ReviewStepProps {
  ...
  onMeta: (patch: Partial<{ name: string; notes: string; priority: number; markStaleAndResolve: boolean }>) => void;
  ...
  onProfileCreated: (kind: NumericsProfileKind, row: NumericsProfileRow) => void;
...
export function ReviewStep({
  ...
  const [previewState, setPreviewState] = useState<"idle" | "computing" | "done" | "failed">("idle");
  ...
  const arithmetic = angleSets ? pointArithmetic(angleSets, resolvedAirfoils.length - symmetricCount, symmetricCount) : null;
  const totalPoints = arithmetic ? arithmetic.points * conditionCount : 0;
  const totalSolverRuns = arithmetic ? arithmetic.solverRuns * conditionCount : 0;
  const totalDerived = arithmetic ? arithmetic.derivedPoints * conditionCount : 0;
  ...
  const previewPayloadKey = useMemo(
    () => JSON.stringify({ plan, airfoilIds: resolvedAirfoils.map((a) => a.id).sort() }),
    [plan, resolvedAirfoils],
  );
  useEffect(() => {
    if (!planReady || !numerics.boundaryProfileId || !numerics.meshProfileId || !numerics.solverProfileId || !numerics.outputProfileId) {
      setPreview(null);
  ...
  const confirmOk = !needsConfirm || (name.trim().length > 0 && confirmText.trim() === name.trim());
  ...
  const summaryRows: Array<[string, string]> = [
    ["Airfoils", `${fCount(resolvedAirfoils.length)}${symmetricCount ? ` (${fCount(symmetricCount)} symmetric)` : ""}`],
    ["Medium", medium?.name ?? "—"],
    ["Conditions", `${fCount(conditionCount)} · ${envelopeSummary}`],
    ["Angles", angleSets ? `${fCount(angleSets.angles.length)} · ${angleSets.angles[0]}° … ${angleSets.angles[angleSets.angles.length - 1]}°` : "—"],
  ...
  return (
    <div data-testid="wizard-review" style={{ display: "grid", gap: 12 }}>
      ...
      <TextField label="Campaign name" value={name} error={issueFor(issues, "Campaign name")} onChange={(v) => onMeta({ name: v })} />
      <TextField label="Notes optional" value={notes} onChange={(v) => onMeta({ notes: v })} />

      <div data-testid="review-summary-table" style={{ border: `1px solid ${C.stroke}`, borderRadius: 8, overflow: "hidden" }}>
        {summaryRows.map(([k, v]) => (
          <div key={k} style={{ display: "grid", gridTemplateColumns: "128px minmax(0, 1fr)", gap: 10, padding: "7px 10px", borderBottom: `1px solid ${C.borderRow}`, fontFamily: MONO, fontSize: 11 }}>
      ...
      <div data-testid="review-reuse-preview" style={{ border: `1px solid ${C.stroke2}`, borderRadius: 8, padding: "9px 11px", display: "grid", gap: 6 }}>
        <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.08em", color: C.dim }}>REUSE PREVIEW</div>
        {!planReady || previewState === "idle" ? (
          ...
          <InfoLine tone="amber" text={`Reuse preview failed: ${previewError} — launching is still safe: already-solved points are never re-run.`} />
        ) : preview?.status === "timeout" ? (
          <InfoLine tone="amber" text="Couldn't compute the reuse preview in time — launching is still safe: already-solved points are never re-run." />
        ) : preview?.status === "ok" ? (
          <div style={{ display: "grid", gap: 6 }}>
            <span style={{ fontFamily: MONO, fontSize: 11, color: C.text }}>
              {fCount(preview.reusedPoints)} of {fCount(preview.totalPoints)} points already solved — they will not re-run · {fCount(preview.totalSolverRuns)} solver runs planned
            </span>
            {allSolved && (
              <div style={{ display: "grid", gap: 6 }}>
                <InfoLine tone="teal" text={`all ${fCount(preview.totalPoints)} points already solved — nothing will run`} />
                <label style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: MONO, fontSize: 11, color: C.text, cursor: "pointer" }}>
                  <input
                    type="checkbox"
                    data-testid="review-mark-stale"
                    checked={markStaleAndResolve}
                    onChange={(e) => onMeta({ markStaleAndResolve: e.target.checked })}
                  />
      ...
      <div style={{ display: "grid", gap: 6 }}>
        <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.08em", color: C.dim }}>NUMERICS</div>
          ...
          {NUMERICS_SLOTS.map((def) => {
            ...
            return (
              <button
                ...
                onClick={() => setExpandedSlot((s) => (s === def.slot ? null : def.slot))}
                style={{
                  display: "grid",
                  gap: 2,
                  textAlign: "left",
              ...
              >
                ...
                <span style={{ color: unresolved ? C.red : C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {current ? `${current.name}${current.isSeeded ? " · seeded" : ""}` : "unresolved — choose"}
                </span>
              </button>
            );
          })}
        ...
        {expandedSlot &&
          (() => {
            ...
            if (rows.length === 0) {
              return (
                <div style={{ display: "grid", gap: 6 }}>
                  <InfoLine tone="amber" text={`No ${def.label.toLowerCase()} profiles exist yet — create the first one to resolve this slot.`} />
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <SelectField
                    ...
                    optionLabels={Object.fromEntries([
                      ["", "choose profile"],
                      ...rows.map((row) => [row.id, `${row.name}${row.isSeeded ? " · seeded" : ""}`]),
                    ])}
                    error={error}
      ...
      <SelectField
        ...
        optionLabels={Object.fromEntries(PRIORITY_OPTIONS.map((o) => [o.value, o.label]))}
        onChange={(v) => onMeta({ priority: Number(v) })}
      />
      <div style={{ fontFamily: MONO, fontSize: 10.5, color: C.dim, lineHeight: 1.5 }}>
        Campaigns above Background pause continuous preset gap-fill until their points are exhausted. On-demand public simulation requests (priority 10) always run first.
      </div>

      <div data-testid="review-queue-context" style={{ display: "grid", gap: 6 }}>
        <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.08em", color: C.dim }}>QUEUE CONTEXT</div>
        {queueError ? (
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            {isProcessDead(queue.sweeper.heartbeatAt) ? (
              ...
              <div data-testid="review-solver-guidance" style={{ display: "grid", gap: 4 }}>
                <InfoLine tone="red" text="Solver process is not running — nothing will be scheduled until it is started." />
                <span style={{ fontFamily: MONO, fontSize: 10, color: C.dim }}>{PROCESS_NOT_RUNNING_DETAIL}</span>
              </div>
            ) : !queue.sweeper.enabled ? (
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <InfoLine tone="amber" text="Sweeper is disabled — nothing will be scheduled until it is enabled." />
                <button
                  type="button"
                  data-testid="review-enable-sweeper"
                  disabled={enablingSweeper}
                  onClick={enableSweeper}
                  style={{ ...ghostBtn, color: C.amber, borderColor: "rgba(245,165,36,.45)", padding: "4px 10px", fontSize: 10.5 }}
                >
            ...
            {queue.engineUnreachableSince && (
              <InfoLine tone="red" text={`Engine unreachable since ${new Date(queue.engineUnreachableSince).toLocaleTimeString()} — no jobs are being submitted.`} />
            )}
          </div>
        )}
      </div>
      ...
      {needsConfirm && (
        <label style={{ display: "block" }} data-admin-field="Launch confirmation">
          <div style={{ fontFamily: MONO, fontSize: 10, color: C.amber, margin: "8px 0 4px" }}>
            {fCount(totalPoints)} points — type the campaign name to confirm the launch
          </div>
          <input
            ...
            style={{ ...inputStyle, borderColor: confirmOk ? C.stroke : C.amber }}
          />
        </label>
      )}
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <button
          ...
          onClick={launch}
          style={{ ...primaryBtn(launching || !confirmOk), padding: "10px 18px" }}
        >
          {launching ? "launching…" : allSolved && !markStaleAndResolve ? "Launch — nothing will run" : `Launch — ${fCount(totalSolverRuns)} solver runs`}
        </button>
        <span style={{ fontFamily: MONO, fontSize: 9, color: C.dimmest }}>idempotency {idempotencyKey.slice(0, 8)}…</span>
      </div>
    </div>
  );
}
