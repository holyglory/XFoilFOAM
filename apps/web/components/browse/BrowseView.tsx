...
import {
  getAirfoilCurveDetail,
  getCategoriesTree,
  getHashtags,
  listAirfoils,
} from "@/lib/api";
import { MetricConditionSelector } from "@/components/MetricConditionSelector";
...
const COLS: {
  ...
  { key: "name", label: "NAME", fmt: (a) => a.name, num: false },
  {
    key: "thicknessPct",
    label: "t/c",
    fmt: (a) => solvedMetric(a.thicknessPct, 1),
    num: true,
  },
  {
    key: "camberPct",
    label: "CAMB",
    fmt: (a) => solvedMetric(a.camberPct, 1),
    num: true,
  },
  {
    key: "areaProfile",
    label: "AREA",
    fmt: (a) => solvedMetric(a.areaProfile, 3),
    num: true,
  },
  {
    key: "ldmax",
    label: "L/D",
    fmt: (a) => solvedMetric(a.ldmax, 1),
    num: true,
  },
  {
    key: "cdmin",
    label: "Cd",
    fmt: (a) => solvedMetric(a.cdmin, 4),
    num: true,
  },
];
...
export function BrowseView({
  ...
  const [conditionKey, setConditionKey] = useState(initialConditionKey);
  ...
  const rootCount = useMemo(
    () => categories.reduce((sum, n) => sum + n.airfoilCount, 0),
    [categories],
  );
  ...
  useEffect(() => {
    setHydrated(true);
    setChoosingDetail(
      new URLSearchParams(window.location.search).get("chooseDetail") === "1",
    );
  }, []);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setVisible(150);
        setSelected((slug) =>
          next.some((a) => a.slug === slug) ? slug : (next[0]?.slug ?? ""),
        );
        setSelectedIds(
          (ids) =>
            new Set([...ids].filter((id) => next.some((a) => a.id === id))),
        );
      } catch (e) {
        if (requestSeq !== browseRequestSeq.current) return;
  ...
  const refreshMeta = async () => {
    const [tree, tagRows] = await Promise.all([
      getCategoriesTree(),
      getHashtags(),
    ]);
    setCategories(tree);
  ...
  const openDetail = (slug: string) => {
    ...
    router.push(
      withMetricCondition(
        `/airfoils/${slug}`,
        conditionKey ||
          items.find((item) => item.slug === slug)?.metricCondition?.key,
      ),
    );
  };
    ...
    <>
      ...
      >
        <main style={{ minWidth: 0 }}>
          ...
          {choosingDetail && (
            <div
              data-testid="choose-detail-state"
              style={{
                fontFamily: MONO,
                fontSize: 11,
                color: C.teal,
          ...
          {advanced && (
            ...
            >
              ...
              <SurfaceFilters
                title="UPPER SURFACE"
                keys={["upperArea", "upperPositive", "upperNegative"]}
                filters={filters}
                setFilters={setFilters}
                focusedMetric={focusedMetric}
                setFocusedMetric={setFocusedMetric}
              ...
              <SurfaceFilters
                title="LOWER SURFACE"
                keys={["lowerArea", "lowerPositive", "lowerNegative"]}
                filters={filters}
                setFilters={setFilters}
                focusedMetric={focusedMetric}
                setFocusedMetric={setFocusedMetric}
              ...
              <SurfaceFilters
                title="MEAN CAMBER"
                keys={["camberArea", "camberPositive", "camberNegative"]}
                filters={filters}
                setFilters={setFilters}
                focusedMetric={focusedMetric}
                setFocusedMetric={setFocusedMetric}
          ...
          {authed && selectedVisibleCount > 0 && (
            <BulkToolbar
              ...
              onArchive={() =>
                window.confirm(
                  `Archive ${selectedVisibleCount} selected airfoils?`,
                ) &&
                act(() =>
                  bulkAirfoils({ ids: [...selectedIds], action: "archive" }),
                )
              }
              onRemove={() =>
                window.confirm(
                  `Remove ${selectedVisibleCount} selected airfoils?`,
                ) &&
                act(() =>
                  bulkAirfoils({ ids: [...selectedIds], action: "remove" }),
                )
              }
              onAssignTags={() =>
                bulkHashtagIds.length > 0 &&
                act(() =>
                  bulkAirfoils({
                    ids: [...selectedIds],
          ...
          >
            ...
            >
              ...
              {COLS.map((c) => (
                <button
                  key={c.key}
                  type="button"
                  className={`airfoil-col-button airfoil-col-${c.key}`}
                  onClick={() => onSort(c.key)}
                  style={sortButton(c.num, sortKey === c.key)}
                >
                  {c.label}
            ...
            {items.slice(0, visible).map((a) => {
              ...
              const checkVisible =
                authed &&
                (checked || selectedIds.size > 0 || hoveredId === a.id);
              return (
                <div
                  key={a.slug}
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    background:
                      selected === a.slug ? C.rowActive : "transparent",
                  }}
                >
                  <button
                    type="button"
                    aria-label={
                      checked ? `Deselect ${a.name}` : `Select ${a.name}`
                    }
                    ...
                    style={{
                      ...
                      pointerEvents: checkVisible ? "auto" : "none",
                    }}
                  >
                  ...
                  <span
                    className="airfoil-cell airfoil-col-thicknessPct"
                    style={{
                      textAlign: "right",
                      color: a.thicknessPct == null ? C.dimmest : C.text,
                    }}
                  >
                    {solvedMetric(a.thicknessPct, 1)}
                  </span>
                  <span
                    className="airfoil-cell airfoil-col-camberPct"
                    style={{
                      textAlign: "right",
                      color: a.camberPct == null ? C.dimmest : C.text,
                    }}
                  >
                    {solvedMetric(a.camberPct, 1)}
                  </span>
                  <span
                    className="airfoil-cell airfoil-col-areaProfile"
                    style={{
                      textAlign: "right",
                      color: a.areaProfile == null ? C.dimmest : C.muted,
                    }}
                  >
                    {solvedMetric(a.areaProfile, 3)}
                  </span>
                  <span
                    ...
                    title={
                      a.ldmax == null
                        ...
                        : a.metricCondition
                          ? `${a.metricsSource === "prediction" ? "NeuralFoil" : "Combined estimate"} · ${conditionLabel(a.metricCondition)}`
                          : `Fitted ${a.fitStatus ?? "polar"}`
                    }
                    style={{
                      textAlign: "right",
                      color: a.ldmax == null ? C.dim : C.teal,
                    }}
                  >
        ...
        <PreviewPane
          slug={selected}
          summary={items.find((item) => item.slug === selected)}
      ...
      >
        {props.keys.map((key) => (
          <RangeFields
            ...
            setFocusedMetric={props.setFocusedMetric}
            min={-0.2}
            max={0.2}
            step={0.001}
          />
...
function RangeFields({
  ...
  const invalid =
    num(filters[minKey]) !== undefined &&
    num(filters[maxKey]) !== undefined &&
    minValue > maxValue;
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        <input
          ...
          style={inputStyle({
            fontSize: 11,
            padding: "7px 8px",
            borderColor: invalid ? C.red : C.stroke,
          })}
        />
        <input
      ...
      {focusedMetric === metric && (
        <div
          ...
          style={{ display: "grid", gap: 5, marginTop: 6 }}
        >
          <input
            type="range"
            min={min}
            max={max}
            step={step}
            value={Math.min(max, Math.max(min, minValue))}
            onChange={(e) => set(minKey, e.target.value)}
          />
          <input
            type="range"
            ...
            max={max}
            step={step}
            value={Math.min(max, Math.max(min, maxValue))}
            onChange={(e) => set(maxKey, e.target.value)}
          />
        </div>
      )}
      {invalid && (
        <div
          style={{ fontFamily: MONO, fontSize: 10, color: C.red, marginTop: 4 }}
        >
          min must be &lt;= max
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        height="110"
        role="img"
        aria-label={`${METRIC_LABELS[metric]} area highlight`}
        style={{ display: "block" }}
      >
        <line
          x1="18"
          ...
          x2="342"
          y2="56"
          stroke="var(--aero-border-rule)"
          strokeWidth="1"
          strokeDasharray="4 4"
        />
        <path
          d="M20 56 C80 20 220 22 340 56 L340 56 C220 72 90 70 20 56 Z"
          fill="rgba(45,212,191,0.08)"
          stroke="var(--aero-dim)"
        />
        {!negative && !lower && !camber && (
          <path
            d="M20 56 C80 20 220 22 340 56 L340 56 Z"
            fill="rgba(45,212,191,0.32)"
          />
        )}
        {!positive && lower && (
          <path
            d="M20 56 C90 70 220 72 340 56 L340 56 Z"
            fill="rgba(245,165,36,0.28)"
          />
        )}
        {camber && (
          <path
            d="M20 56 C110 46 215 46 340 56"
            fill="none"
            stroke="var(--aero-amber)"
            strokeWidth="3"
          />
        )}
        {positive && (
          <path
            d="M20 56 C90 34 225 34 340 56 L340 56 Z"
            fill="rgba(45,212,191,0.34)"
          />
        )}
        {negative && (
          <path
            d="M20 56 C90 72 220 72 340 56 L340 56 Z"
...
function BulkToolbar(props: {
  ...
  const filteredCategories = props.categories.filter((c) =>
    `${c.name} ${c.path}`.toLowerCase().includes(categoryQuery.toLowerCase()),
  );
  const filteredTags = props.hashtags.filter((h) =>
    `${h.name} ${h.slug}`.toLowerCase().includes(tagQuery.toLowerCase()),
  );
  const selectedTagNames = props.hashtags
    .filter((h) => props.hashtagIds.includes(h.id))
    .map((h) => h.name);
  const toggleTag = (id: string) => {
    props.setHashtagIds(
      props.hashtagIds.includes(id)
        ? props.hashtagIds.filter((x) => x !== id)
        : [...props.hashtagIds, id],
    );
  };
  ...
  return (
    <div data-testid="bulk-toolbar" className="bulk-toolbar">
        ...
        {moveOpen && (
          <div className="bulk-popover" data-testid="bulk-move-popover">
            ...
            {filteredCategories.map((c) => (
              ...
              >
                <span style={{ color: C.dim }}>{"  ".repeat(c.depth)}</span>
                {c.name}
              </button>
            ))}
          </div>
        ...
        <input
          ...
          placeholder={
            selectedTagNames.length
              ? selectedTagNames.map((name) => `#${name}`).join(", ")
              : "hashtags..."
          }
          style={selectStyle}
        />
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: on ? C.rowActive : "transparent",
                    border: "none",
                    borderBottom: `1px solid ${C.borderRow}`,
                    padding: "7px 4px",
                    cursor: "pointer",
      ...
      <button
        type="button"
        disabled={props.busy || props.hashtagIds.length === 0}
        title={
          props.hashtagIds.length === 0
            ? "Choose at least one hashtag first"
            : "Assign selected hashtags"
        }
        onClick={props.onAssignTags}
        style={toolbarBtn}
      >
      ...
      <button
        ...
        style={toolbarBtn}
        aria-label="Remove selected hashtags"
        title={
          props.hashtagIds.length === 0
            ? "Choose at least one hashtag first"
            : "Remove selected hashtags"
        }
      >
      ...
      <button
        ...
        style={toolbarBtn}
        aria-label="Archive selected airfoils"
        title="Archive"
      >
      ...
      <button
        ...
        style={{ ...toolbarBtn, color: C.red }}
        aria-label="Remove selected airfoils"
        title="Remove"
      >
...
function PreviewPane({
  ...
  useEffect(() => {
    ...
    setDetail(null);
    setError(null);
    getAirfoilCurveDetail(slug)
      .then((data) => {
        if (!cancel) setDetail(data);
      })
      .catch((failure) => {
        if (!cancel) setError(String(failure));
      });
    return () => {
      cancel = true;
    };
  ...
  const selectedCondition = conditionKey || summary?.metricCondition?.key;
  const series = detail.progressivePolars?.find(
    (item) => item.conditionKey === selectedCondition,
  );
  const method =
    series?.kind === "estimate"
      ? "Combined estimate"
      : series
        ? "NeuralFoil"
        : "Unavailable";
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "4px 14px 12px" }}>
        {summary?.metricCondition && (
          <div style={{ color: C.muted, fontSize: 11, marginBottom: 10 }}>
            {conditionLabel(summary.metricCondition)}
          </div>
        )}
        {mrows.map(([k, v]) => (
          <div
        ...
        <PolarMiniChart series={series} />
        <Link
          href={withMetricCondition(
            `/airfoils/${detail.slug}`,
            selectedCondition,
          )}
          style={{
            display: "block",
        ...
        <Link
          href={withMetricCondition(
            `/compare?airfoil=${encodeURIComponent(detail.slug)}`,
            selectedCondition,
          )}
          style={{
            display: "block",
