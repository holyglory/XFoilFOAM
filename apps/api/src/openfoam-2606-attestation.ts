...
const receiptJobSchema = z
  .object({
    ...
    points: z.array(receiptPointSchema).min(1),
    remote_render_proof: remoteRenderProofSchema,
  })
  .strict();

export const openCfd2606CanaryReceiptSchema = z
  .object({
...
export async function attestOpenCfd2606CanaryReceipt(
  ...
  const receipt = validateOpenCfd2606CanaryReceiptShape(receiptValue);
  ...
  assertOpenCfd2606EvidenceStorageContract(receipt.evidence_storage, health);
  assertSameRuntime(
    assertOfficialRuntime(receipt.runtime, "canary receipt runtime"),
    liveRuntime,
