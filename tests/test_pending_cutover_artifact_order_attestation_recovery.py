...
WRAPPER = ROOT / "scripts/deploy/recover-pending-opencfd2606-artifact-order-attestation.sh"
MANIFEST = ROOT / "scripts/deploy/deployment-source-manifest.py"
TARGET_REVISION = "d" * 40
BASE_REVISION = "54ecd8f7fa21e6fb90b20ab06769aa7362224be0"
...
if [[ "$mode" == "attest-then-fail" ]]; then exit 15; fi
exit 14
"""
...
def _fixture_sources(
    ...
    for root in (bound, base):
        ...
        _write(root / "apps/api/src/openfoam-2606-attestation.ts", "old ordered attestation\n")
        _write(root / "apps/api/test/openfoam-2606-attestation.test.ts", "old ordered attestation test\n")
        _write(root / "apps/api/src/engine-cutover-routes.ts", "fixed bounded route\n")
        _write(root / "apps/api/test/engine-cutover-routes.test.ts", "fixed bounded route test\n")
        _write(root / "apps/api/src/admin-routes.ts", "fixed admin routes\n")
    ...
    _write(target / "apps/api/src/openfoam-2606-attestation.ts", "new order-independent attestation\n")
    _write(target / "apps/api/test/openfoam-2606-attestation.test.ts", "new order-independent attestation test\n")
    _write(
        target / "tests/test_pending_cutover_artifact_order_attestation_recovery.py",
        "# additive recovery harness\n",
    )
    base_file_count = len(_inventory(base).splitlines())

    wrapper = WRAPPER.read_text()
    replacements = {
        "8cfa881e332c9f332f8cc8a229082a3465debfd3758912cb3afb8463140f0340": BASE_TREE,
        'EXPECTED_BASE_SOURCE_FILE_COUNT="2210"': f'EXPECTED_BASE_SOURCE_FILE_COUNT="{base_file_count}"',
        ...
        "4ddb1ed4c99b8349340209a76a754e7fd6ae202ef11df3d06012e8a3f6f97d42": env_before_sha,
        "8421c15692afc6e36a834e481734411db61ecf18a95ce4db3a46ec9c7f7ad96c": db_before_sha,
        "6af5dffab919158713f7eecb9a04d23bdb06165b90e5ba9231ba3946c81a594e": failed_journal_sha,
        ...
        "sha256:4ee79b1da45fdb5e9cfea63c2cd25380e9ba0bf858cebfc23a1c74ec68f8f964": OLD_NODE_IMAGE,
        ...
        "5f82ec56d030f54b4dfbe168f93d47a9bdc8e2c2bd1ad38faa31fbc08865425d": OLD_NODE_CONTAINER,
        ...
        "928986cd328e7af647cefe7c241ed1a5ce9a6446907061055a28f28392c0944e": _sha(base / "apps/api/src/openfoam-2606-attestation.ts"),
        "25452e17a7714911151cb13c5f88e2b91485a126387baf3514080e9f3ac1bb44": _sha(base / "apps/api/test/openfoam-2606-attestation.test.ts"),
        "6f10619510378451e47e8aaa6579e663b879e47ce0af98097680c2b4462ddc62": _sha(target / "apps/api/src/openfoam-2606-attestation.ts"),
        "6046af2febe268060afcf7eff386a99ab5c6d0930e5eea2d70f10a715805b65b": _sha(target / "apps/api/test/openfoam-2606-attestation.test.ts"),
        ...
        "e3e1782f0517ea29e451fd89661a1a54f982673cd62ad5502e5d45eaaa6a94f4": _sha(target / "apps/api/src/admin-routes.ts"),
    ...
    _write(
        target / "scripts/deploy/recover-pending-opencfd2606-artifact-order-attestation.sh",
        wrapper,
        executable=True,
    )
    ...
    assert (
        "16fbe786c7c6346667c683deb790a5feaff773bbf0e1e2d7de88a0ceb1aff76c"
        in wrapper
    )
    ...
    _write(
        artifacts / "opencfd2606-artifact-order-base-ls-tree.txt",
        base_inventory,
    )
    _write(
        artifacts / "opencfd2606-artifact-order-target-ls-tree.txt",
        target_inventory,
    )
...
if [[ "${1:-}" == "image" && "${2:-}" == "inspect" ]]; then
  ...
  case "$tag" in
    ...
    *node-api-artifact-order-rollback*) cat "$ROLLBACK_TAG_FILE" ;;
    *node-api-artifact-order*) cat "$REPAIR_TAG_FILE" ;;
    sha256:*) printf '%s' "$tag" ;;
    *) exit 1 ;;
  esac
  printf '\n'; exit 0
...
if [[ "${1:-}" == "exec" ]]; then
  ...
  if [[ "$joined" == *" sha256sum /app/apps/api/src/openfoam-2606-attestation.ts /app/apps/api/test/openfoam-2606-attestation.test.ts "* ]]; then
    if [[ "$(cat "$NODE_SOURCE_FILE")" == "new" ]]; then
      printf '%s  /app/apps/api/src/openfoam-2606-attestation.ts\n' "$NEW_ATTESTATION_SHA"
      printf '%s  /app/apps/api/test/openfoam-2606-attestation.test.ts\n' "$NEW_ATTESTATION_TEST_SHA"
    else
      printf '%s  /app/apps/api/src/openfoam-2606-attestation.ts\n' "$OLD_ATTESTATION_SHA"
      printf '%s  /app/apps/api/test/openfoam-2606-attestation.test.ts\n' "$OLD_ATTESTATION_TEST_SHA"
    fi
    exit 0
  fi
  ...
  if [[ "$joined" == *" sha256sum /app/apps/api/test/engine-cutover-routes.test.ts "* ]]; then
...
if [[ "${1:-}" == "tag" ]]; then
  ...
  case "$source" in
    ...
    *node-api-artifact-order*) value="$(cat "$REPAIR_TAG_FILE")" ;;
    *) value="$source" ;;
  esac
  case "$target" in
    app-node-api) printf '%s' "$value" >"$APP_TAG_FILE" ;;
    *rollback*) printf '%s' "$value" >"$ROLLBACK_TAG_FILE" ;;
    *artifact-order*) printf '%s' "$value" >"$REPAIR_TAG_FILE" ;;
  esac
  exit 0
fi
if [[ "${1:-}" == "build" ]]; then
...
if [[ "$joined" == *"127.0.0.1:4000/api/admin/me"* ]]; then
  printf '%s' '{"authed":true,"email":"cutover-artifact-order@airfoils.pro"}'
  exit 0
fi
...
def harness(tmp_path: Path) -> tuple[dict[str, str], Path, dict[str, Path], Path]:
    ...
    failed = state / "pending-opencfd2606-attestation-413-recovery.json"
    failed.write_text(
        json.dumps(
            {
                "schemaVersion": 1,
                "purpose": "pending-opencfd2606-attestation-413-recovery",
                "status": "failed",
                "failureCount": 1,
                "lastExitCode": 14,
                "repairSourceRevision": BASE_REVISION,
                "repairSourceTreeSha256": BASE_TREE,
                "repairSourceFileCount": 2210,
                "currentDeploymentEnvironmentSha256": env_before_sha,
                "currentDatabaseSnapshotSha256": db_before_sha,
                "currentCanaryReceiptSha256": receipt_sha,
                "currentCutoverStateKind": "pending-receipt",
                "promotionEligible": False,
                "nodeApiImageAfter": OLD_NODE_IMAGE,
                "nodeApiContainerAfter": OLD_NODE_CONTAINER,
                "apiImage": API_IMAGE,
                "apiContainer": API_CONTAINER,
    ...
    env = {
        **os.environ,
        ...
        "OLD_ATTESTATION_SHA": _sha(target.parent / "base/apps/api/src/openfoam-2606-attestation.ts"),
        "OLD_ATTESTATION_TEST_SHA": _sha(target.parent / "base/apps/api/test/openfoam-2606-attestation.test.ts"),
        "NEW_ATTESTATION_SHA": _sha(target / "apps/api/src/openfoam-2606-attestation.ts"),
        "NEW_ATTESTATION_TEST_SHA": _sha(target / "apps/api/test/openfoam-2606-attestation.test.ts"),
        "ROUTE_SHA": _sha(target / "apps/api/src/engine-cutover-routes.ts"),
        "ROUTE_TEST_SHA": _sha(target / "apps/api/test/engine-cutover-routes.test.ts"),
        "COOKIE_VALUE": COOKIE,
        "ADMIN_ROUTES_SHA": _sha(target / "apps/api/src/admin-routes.ts"),
...
def test_exact_retained_receipt_is_attested_without_engine_rebuild(
    ...
    journal = json.loads(
        (state / "pending-opencfd2606-artifact-order-attestation-recovery.json").read_text()
    )
...
def test_attested_partial_exit_is_reconciled_without_replaying_child(
    harness: tuple[dict[str, str], Path, dict[str, Path], Path],
) -> None:
    env, wrapper, files, state = harness
    env["FAKE_CHILD_MODE"] = "attest-then-fail"
    ...
    journal = json.loads(
        (
            state
            / "pending-opencfd2606-artifact-order-attestation-recovery.json"
        ).read_text()
    )
    assert journal["status"] == "attested_awaiting_continuation"
    assert journal["promotionEligible"] is False
...
def test_source_tamper_is_rejected_before_runtime_mutation(
    ...
    target_attestation.write_text(target_attestation.read_text() + "tamper\n")
...
@pytest.mark.parametrize(
    ("artifact", "message"),
    [
        ("receipt", "initial live state differs"),
        ("journal", "failed recovery journal differs"),
    ],
)
def test_pinned_incident_artifact_tamper_is_non_mutating(
    harness: tuple[dict[str, str], Path, dict[str, Path], Path],
    artifact: str,
    message: str,
) -> None:
    env, wrapper, files, state = harness
    path = (
        state / "openfoam-2606-canary-receipt.pending.json"
        if artifact == "receipt"
        else state / "pending-opencfd2606-attestation-413-recovery.json"
    )
...
def test_production_workflow_and_wrapper_contract_are_narrow() -> None:
    ...
    assert "pending_cutover_artifact_order_attestation_recovery:" in workflow
    assert "recover-pending-opencfd2606-artifact-order-attestation.sh" in workflow
    assert "opencfd2606-artifact-order-base-ls-tree.txt" in workflow
    assert "opencfd2606-artifact-order-target-ls-tree.txt" in workflow
    assert "PENDING_CUTOVER_ARTIFACT_ORDER_ATTESTATION_RECOVERY" in workflow
    assert workflow.count(
        "RECOVERY_ACTIONS=$((RECOVERY_ACTIONS + 1))"
    ) == 7
    assert (
        "if ((RECOVERY_ACTIONS > 1)); then" in workflow
        and "Select only one pending-cutover recovery action" in workflow
    )
    assert (
        "if: ${{ env.PENDING_CUTOVER_ARTIFACT_ORDER_ATTESTATION_RECOVERY == 'true' }}"
        in workflow
    )
