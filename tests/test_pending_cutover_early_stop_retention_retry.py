...
WRAPPER = ROOT / "scripts/deploy/retry-pending-opencfd2606-early-stop-retention.sh"
PRIOR_WRAPPER = ROOT / "scripts/deploy/retry-pending-opencfd2606-transient-retention.sh"
...
GRANDPARENT_REVISION = "cd0967a1ba4ef82113d6b1eae9e38f0a7baec3a2"
NODE_SOURCE_REVISION = "1f815f89c523cbf667725c1cd681d729c06a10c9"
PRIOR_TREE = "7" * 64
TARGET_REVISION = "d" * 40
OLD_BUILD = "prod-20260717-2ab861cb4ce6-r4"
TARGET_BUILD = f"prod-20260717-{TARGET_REVISION[:12]}-r5"
...
PRIOR_NODE_CONTAINER = "1" * 64
PRIOR_API_CONTAINER = "a" * 64
PRIOR_WORKER_CONTAINER = "b" * 64
...
def _fixture_sources(tmp_path: Path) -> tuple[Path, Path, Path, str, int, str, str]:
    ...
    _write(base / "scripts/deploy/retry-pending-opencfd2606-transient-retention.sh", PRIOR_WRAPPER.read_bytes(), executable=True)
    ...
    _write(target / "src/airfoilfoam/pipeline.py", (ROOT / "src/airfoilfoam/pipeline.py").read_bytes() + b"\n# early-stop retention pipeline fixture delta\n")
    _write(target / "src/airfoilfoam/retention.py", (ROOT / "src/airfoilfoam/retention.py").read_bytes() + b"\n# early-stop retention fixture delta\n")
    _write(target / "tests/test_retention.py", (ROOT / "tests/test_retention.py").read_bytes() + b"\n# early-stop retention test fixture delta\n")
    wrapper_text = WRAPPER.read_text()
    _write(target / "scripts/deploy/retry-pending-opencfd2606-early-stop-retention.sh", wrapper_text, executable=True)
    _write(target / "tests/test_pending_cutover_early_stop_retention_retry.py", "# early-stop retry fixture contract\n")
    inventory_path = target / ".codex-artifacts/opencfd2606-early-stop-retention-base-ls-tree.txt"
    ...
    replacements = {
        ...
        "EXPECTED_PRIOR_ENGINE_SOURCE_TREE_SHA256=\"e8f5ecf9e9500d9de20da521fb9ea7edb9696be548bd3c667292019f36079c79\"": f'EXPECTED_PRIOR_ENGINE_SOURCE_TREE_SHA256="{PRIOR_TREE}"',
        "EXPECTED_BASE_GIT_TREE_INVENTORY_SHA256=\"cfc14fb3990219b323e9349a2054734ec9ef72f638d44658f50cfc89b8f258dd\"": f'EXPECTED_BASE_GIT_TREE_INVENTORY_SHA256="{hashlib.sha256(inventory).hexdigest()}"',
        "EXPECTED_PIPELINE_SHA256=\"18c2d85d8e3202c7a4b3ab41bc776c9c9588a2528f016d3be7ba08323cb7c01a\"": f'EXPECTED_PIPELINE_SHA256="{_sha(target / "src/airfoilfoam/pipeline.py")}"',
        "EXPECTED_RETENTION_SHA256=\"3e73c52ce46583d37642b21c6bb3b99c124c399b0f50289e8e583b494a7a39f1\"": f'EXPECTED_RETENTION_SHA256="{_sha(target / "src/airfoilfoam/retention.py")}"',
        "EXPECTED_RETENTION_TEST_SHA256=\"8def32a45b558c07e597d281cb6e8ed7eb51e8bbd6f91e422da16c1e20c3c8e6\"": f'EXPECTED_RETENTION_TEST_SHA256="{_sha(target / "tests/test_retention.py")}"',
        ...
        "EXPECTED_PRIOR_WRAPPER_SHA256=\"dac5b9778de7b911fa869ead312ce4e480d7a0283d773a15b909f18874776372\"": f'EXPECTED_PRIOR_WRAPPER_SHA256="{_sha(target / "scripts/deploy/retry-pending-opencfd2606-transient-retention.sh")}"',
        ...
        "EXPECTED_INITIAL_NODE_CONTAINER=\"4f61a8f4112cecda0e9c640f9a06ef2225090a1a43e9ed466fed9fa3b97e4dfc\"": f'EXPECTED_INITIAL_NODE_CONTAINER="{NODE_CONTAINER}"',
        "EXPECTED_INITIAL_API_CONTAINER=\"67f4b36e01f5008fa2a4c4f11bc79d244fcbda7a6bedbe37c78cb292c38307c7\"": f'EXPECTED_INITIAL_API_CONTAINER="{API_CONTAINER}"',
        "EXPECTED_INITIAL_WORKER_CONTAINER=\"ff296834c225c8b978d97768139e7efbc02b15b4f48d35b925ad94655b34b3c4\"": f'EXPECTED_INITIAL_WORKER_CONTAINER="{WORKER_CONTAINER}"',
        "EXPECTED_INITIAL_API_IMAGE=\"sha256:93e2dff0d644856c80e59d6113d5046700796bf10d260ccf8d69b8a03a5fd57d\"": f'EXPECTED_INITIAL_API_IMAGE="{OLD_API_IMAGE}"',
        "EXPECTED_INITIAL_WORKER_IMAGE=\"sha256:8c2c98eba02b1f18ad8b4f2600b5d167c725e813bb313124f95fd0855d604941\"": f'EXPECTED_INITIAL_WORKER_IMAGE="{OLD_WORKER_IMAGE}"',
    }
    ...
    _write(target / "scripts/deploy/retry-pending-opencfd2606-early-stop-retention.sh", wrapper_text, executable=True)
    target_tree, target_count = _seal(target, TARGET_REVISION)
    return base, target, target / "scripts/deploy/retry-pending-opencfd2606-early-stop-retention.sh", base_tree, base_count, target_tree, hashlib.sha256(inventory).hexdigest()
...
case "$url" in
  ...
  */api/admin/me) printf '{"authed":true,"email":"cutover-early-stop-retention@airfoils.pro"}\n';;
  *) exit 2;;
esac
''', executable=True)
    _write(fake_bin / "mktemp", r'''#!/usr/bin/env bash
set -euo pipefail
if [[ " $* " == *" -d /dev/shm/airfoils-opencfd2606-early-stop-retention.XXXXXX "* ]]; then
...
def harness(tmp_path: Path) -> tuple[dict[str, str], Path, Path, dict[str, Path]]:
    ...
    _replace_env(
        source_bound_env,
        {
            "AIRFOILFOAM_BUILD_ID": "prod-20260717-cd0967a1ba4e-r3",
            "ENGINE_EXPECTED_BUILD_ID": "prod-20260717-cd0967a1ba4e-r3",
        },
    )
    ...
    failed = state / "pending-opencfd2606-transient-retention-retry.json"
    failed_payload = {
        "schemaVersion": 1, "purpose": "pending-opencfd2606-transient-retention-retry", "status": "failed",
        "preparedAt": "2026-07-17T00:00:00+00:00", "updatedAt": "2026-07-17T00:05:00+00:00",
        "completedAt": None, "failureCount": 1, "lastExitCode": 137,
        "priorFailedRetentionJournalSha256": "a" * 64,
        "boundReleaseSourceRevision": BOUND_REVISION, "boundReleaseSourceTreeSha256": base_tree,
        "priorEngineSourceRevision": GRANDPARENT_REVISION,
        ...
        "nodeApiContainerBefore": PRIOR_NODE_CONTAINER, "nodeApiContainerAfter": NODE_CONTAINER,
        "apiContainerBefore": PRIOR_API_CONTAINER, "workerContainerBefore": PRIOR_WORKER_CONTAINER,
    ...
    replacements = {
        ...
        'EXPECTED_INITIAL_ENV_SHA256="4caf2f7c7f40a1b3730cfddac5502a0b546db1308fa656cac95279d16bcfdb87"': f'EXPECTED_INITIAL_ENV_SHA256="{_sha(env_file)}"',
        'EXPECTED_PRIOR_SOURCE_BOUND_ENV_SHA256="9fbf51d7ae4455d3ff92c0b49876243e356f80fc3f9789368bb4fcc4809b84f9"': f'EXPECTED_PRIOR_SOURCE_BOUND_ENV_SHA256="{_sha(source_bound_env)}"',
        ...
        'EXPECTED_PRIOR_TRANSIENT_RETENTION_JOURNAL_SHA256="f43b6b1621a41c2aae211a20ea163c2cc131df7a47252df80463190402bd390b"': f'EXPECTED_PRIOR_TRANSIENT_RETENTION_JOURNAL_SHA256="{_sha(failed)}"',
        'EXPECTED_PRIOR_FAILED_RETENTION_JOURNAL_SHA256="b99ca862100542593f7d8fc24dd47b9d62eb3e4d657dbc1c1590ce6c90050224"': f'EXPECTED_PRIOR_FAILED_RETENTION_JOURNAL_SHA256="{failed_payload["priorFailedRetentionJournalSha256"]}"',
        'EXPECTED_PRIOR_ENGINE_APPLICATION_SOURCE_SHA256="57bca7b4fee964f30d0a44a9a7e83967ed975357aee1f44f46d0d3865412439c"': f'EXPECTED_PRIOR_ENGINE_APPLICATION_SOURCE_SHA256="{prior_app_source}"',
        'EXPECTED_PRIOR_ENGINE_SOURCE_FILE_COUNT="2206"': f'EXPECTED_PRIOR_ENGINE_SOURCE_FILE_COUNT="{base_count}"',
        'EXPECTED_PRIOR_TRANSIENT_ENV_BEFORE_SHA256="a34fa01374905d4f037fb36ac0ca2981b89443c80d9ae6896bdb9f788608481a"': f'EXPECTED_PRIOR_TRANSIENT_ENV_BEFORE_SHA256="{failed_payload["deploymentEnvironmentBeforeSha256"]}"',
        'EXPECTED_PRIOR_TRANSIENT_NODE_BEFORE="23f5f7b9ed07eea74f7eca57247fd9cf03ebf9c179620f3fc3e628be806b2016"': f'EXPECTED_PRIOR_TRANSIENT_NODE_BEFORE="{PRIOR_NODE_CONTAINER}"',
        'EXPECTED_PRIOR_TRANSIENT_API_BEFORE="8c09f2cf019c568b459fcdb7af595a5e13ad8c59f0ed55ff58f34bb506b848a5"': f'EXPECTED_PRIOR_TRANSIENT_API_BEFORE="{PRIOR_API_CONTAINER}"',
        'EXPECTED_PRIOR_TRANSIENT_WORKER_BEFORE="41ae0007b8c19f0d29616d7304b86a38f23346b3940a5178f565dfe2b9ff931f"': f'EXPECTED_PRIOR_TRANSIENT_WORKER_BEFORE="{PRIOR_WORKER_CONTAINER}"',
    }
    for old, new in replacements.items():
    ...
    target_inventory_path = target / ".codex-artifacts/opencfd2606-early-stop-retention-target-ls-tree.txt"
...
def _refresh_target_inventory(target: Path, env: dict[str, str]) -> None:
    ...
    path = target / ".codex-artifacts/opencfd2606-early-stop-retention-target-ls-tree.txt"
    _write(path, payload)
    path.chmod(0o600)
...
def test_retry_builds_from_target_snapshot_reuses_node_and_stops_at_attestation(harness) -> None:
    ...
    prior_journal = state / "pending-opencfd2606-transient-retention-retry.json"
    prior_journal_bytes = prior_journal.read_bytes()
    prior_wrapper = Path(env["STAGING_DIR"]) / "scripts/deploy/retry-pending-opencfd2606-transient-retention.sh"
    ...
    payload = json.loads((state / "pending-opencfd2606-early-stop-retention-retry.json").read_text())
    ...
    assert payload["priorFailedTransientRetentionJournalSha256"] == hashlib.sha256(prior_journal_bytes).hexdigest()
...
def test_failed_child_is_journaled_and_exact_retry_resumes_without_node_build(harness) -> None:
    ...
    assert json.loads((state / "pending-opencfd2606-early-stop-retention-retry.json").read_text())["status"] == "attested_awaiting_continuation"
    assert files["node-tag"].read_text() == NODE_IMAGE
...
@pytest.mark.parametrize(
    "mutation",
    ["journal", "base_inventory", "target_inventory", "unrelated", "missing_delta", "wrong_allowed_bytes", "critical", "env", "source_bound_env_alias", "database", "api_image", "node_container"],
)
def test_retry_rejects_wrong_incident_or_source_before_runtime_mutation(harness, mutation: str) -> None:
    ...
    prior_wrapper = target / "scripts/deploy/retry-pending-opencfd2606-transient-retention.sh"
    prior_wrapper_bytes = prior_wrapper.read_bytes()
    if mutation == "journal":
        prior_journal.write_text(prior_journal.read_text() + " ")
    elif mutation == "base_inventory":
        path = target / ".codex-artifacts/opencfd2606-early-stop-retention-base-ls-tree.txt"; path.write_text(path.read_text() + " ")
    elif mutation == "target_inventory":
        path = target / ".codex-artifacts/opencfd2606-early-stop-retention-target-ls-tree.txt"; path.write_text(path.read_text() + " ")
    ...
    elif mutation == "source_bound_env_alias":
        current_env_sha = _sha(state / ".env.deploy")
        path = wrapper
        text = path.read_text()
        text = text.replace(
            next(line for line in text.splitlines() if line.startswith("EXPECTED_PRIOR_SOURCE_BOUND_ENV_SHA256=")),
            f'EXPECTED_PRIOR_SOURCE_BOUND_ENV_SHA256="{current_env_sha}"',
        )
    ...
    assert not (state / "pending-opencfd2606-early-stop-retention-retry.json").exists()
...
def test_production_wrapper_and_workflow_contract_are_narrow() -> None:
    ...
    assert "pending-opencfd2606-early-stop-retention-retry.json" in wrapper
    ...
    assert "18c2d85d8e3202c7a4b3ab41bc776c9c9588a2528f016d3be7ba08323cb7c01a" in wrapper
    assert "3e73c52ce46583d37642b21c6bb3b99c124c399b0f50289e8e583b494a7a39f1" in wrapper
    assert "8def32a45b558c07e597d281cb6e8ed7eb51e8bbd6f91e422da16c1e20c3c8e6" in wrapper
    assert "f43b6b1621a41c2aae211a20ea163c2cc131df7a47252df80463190402bd390b" in wrapper
    assert "4caf2f7c7f40a1b3730cfddac5502a0b546db1308fa656cac95279d16bcfdb87" in wrapper
    assert "9fbf51d7ae4455d3ff92c0b49876243e356f80fc3f9789368bb4fcc4809b84f9" in wrapper
    assert "4caf2f7c7f40a1b3730cfddac5502a0b546db1308fa656cac95279d16bcfdb87" != "9fbf51d7ae4455d3ff92c0b49876243e356f80fc3f9789368bb4fcc4809b84f9"
    assert WRAPPER.stat().st_mode & 0o111
    assert "pending_cutover_early_stop_retention_retry:" in workflow
    assert "retry-pending-opencfd2606-early-stop-retention.sh" in workflow
    assert "opencfd2606-early-stop-retention-base-ls-tree.txt" in workflow
    assert "opencfd2606-early-stop-retention-target-ls-tree.txt" in workflow
    assert PRIOR_WRAPPER.read_bytes() == subprocess.check_output(
        ["git", "show", f"{BASE_REVISION}:scripts/deploy/retry-pending-opencfd2606-transient-retention.sh"], cwd=ROOT
    )
